...
'メッセージを送る
'Public Declare Function SendMessage Lib "user32.dll" Alias "SendMessageA" _                        '2016.01.06
'(ByVal hwnd As Long, ByVal MSG As Long, ByVal wParam As Long, ByVal lParam As Long) As Long        '2016.01.06
...
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>    他のアプリケーションのウィンドウを操作する  2016.10.20
...
ByVal fAttach As Long) As Long '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>    他のアプリケーションのウィンドウを操作する  2016.10.20
