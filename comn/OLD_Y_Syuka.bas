...
'********************************************************************
'ファイルＩＤ
Public Const OLD_Y_SYU_ID$ = "OLD_Y_SYU"

'ページサイズ
Public Const OLD_Y_SYU_PG_SIZ% = 2048

'ポジション・ブロック
Public OLD_Y_SYU_POS    As POSBLK
'********************************************************************
'*
'*                           構造体定義
'*
...
'データ・バッファ
Public OLD_Y_SYUREC             As OLD_Y_SYUREC_Tag

'キー定義
Type KEY0_OLD_Y_SYU            'ＫＥＹ０
    JGYOBU(0 To 0)              As Byte     '事業部区分
    KEY_ID_NO(0 To 7)           As Byte     'ID-NO
End Type
'キー・データ
Public K0_OLD_Y_SYU                 As KEY0_OLD_Y_SYU

Function OLD_Y_SYU_Open(Mode As Integer) As Integer
'********************************************************************
'*
'*              （旧）出荷予定データ  ＯＰＥＮ
'*
...
Dim FullPath    As String
    ...
    OLD_Y_SYU_Open = True
                                            '出荷予定データフルパス取込み
    sts = GetIni("FILE", OLD_Y_SYU_ID, "CONV2006", c)
    If sts <> False Then
        Call Log_Out(LOG_F, "CONV2006.INI [OLD_Y_SYU_ID]読み込みエラー ")
        Exit Function
    End If
    FullPath = RTrim(c)
    
    Do
        sts = BTRV(BtOpOpen, OLD_Y_SYU_POS, OLD_Y_SYUREC, Len(OLD_Y_SYUREC), ByVal FullPath, Len(FullPath), Mode)
        Select Case sts
            Case BtNoErr
                Exit Do
            ...
            Case BtErrFileNotFound
                ...
                Exit Function
            Case Else
                Call File_Error(sts, BtOpOpen, "（旧）出荷予定データ")
                Exit Function
        End Select
    Loop
    OLD_Y_SYU_Open = False
End Function
