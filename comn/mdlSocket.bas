...
Public Const MAX_FSENDDATASIZE  As Integer = 1446                   '最大ファイル送信データサイズ
...
' return    :   なし
'*******************************************************************************
Public Sub WriteLogMsg(ByVal strMsg As String, ByVal strFunc As String, Optional ByVal intID As Integer = -1, Optional lngIcon As LogMsgIcon = icoMessage)
    ...
    '----- ログ出力文字列を作成 -----
    strLogText = "[" & strNow & "]" & vbTab & CStr(varIcon(lngIcon - 1)) & vbTab & "(" & strID & ")" & vbTab & "【" & strFunc & "】" & vbTab & strMsg
