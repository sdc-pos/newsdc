...
Dim sts As Integer
    ...
    For i = 0 To UBound(JGYOBU_T)
        ...
        If JGYOBU_T(i).CODE = Last_JGYOBU Then
            F1030601.Caption = "出荷確認（" + RTrim(JGYOBU_T(i).NAME) + ")"
...
Dim i As Integer
    ...
    Next i
                                    '事業部切り替え
    F1030601.Caption = "出荷確認（" + RTrim(JGYOBU_T(Index).NAME) + ")"
...
'    Call Input_Lock
    ...
    Call UniCode_Conv(K2_Y_SYU.JGYOBU, Last_JGYOBU) '事業部
                                                    ...
                                                    '注文区分
    Call UniCode_Conv(K2_Y_SYU.KEY_CYU_KBN, Right(Combo(pcmbCYU_KBN).Text, 1))
                                                    '向け先
...
'----------------------------------------------------------------------------
    ...
    F1030601.MousePointer = vbHourglass

    Call Ctrl_Lock(F1030601)
...
'----------------------------------------------------------------------------
    ...
    Call Ctrl_UnLock(F1030601)


    F1030601.MousePointer = vbDefault
...
Dim sts As Integer
    ...
    Select Case StrConv(Y_SYUREC.CYU_KBN, vbUnicode)                '注文区分
            Case CYU_KBN_TUK
...
If Not IsNumeric(StrConv(Y_SYUREC.JITU_SURYO, vbUnicode)) Then
    ...
    Call UniCode_Conv(Y_SYUREC.JITU_SURYO, "0")
