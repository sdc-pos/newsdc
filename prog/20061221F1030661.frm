...
Private Const poptSEL_ETC% = 1          'その他選択



Private Const ptxSEL_OKURI_NO% = 0      '問合せ№
Private Const ptxSEL_SYUKA_YY% = 1      '出荷日(年)
Private Const ptxSEL_SYUKA_MM% = 2      '出荷日(年)
Private Const ptxSEL_SYUKA_DD% = 3      '出荷日(年)

Private Const ptxSEL_MUKE_CODE% = 4     '出荷先
Private Const ptxSEL_HIN_NO% = 5        '品番
...
Dim UNSOU_KAISHA    As Variant
    ...
    For i = 0 To UBound(JGYOBU_T)
        ...
        If JGYOBU_T(i).CODE = Last_JGYOBU Then
            F1030661.Caption = "出荷問合わせ(問合せ№)　検品取り消し機能付き（" + RTrim(JGYOBU_T(i).NAME) + ")"
...
Dim i As Integer
    ...
    Next i
                                    '事業部切り替え
    F1030661.Caption = "出荷問い合わせ(問合せ№)　検品取り消し機能付き（" + RTrim(JGYOBU_T(Index).NAME) + ")"
...
Dim yn  As Integer
    ...
    Else
        ...
        yn = MsgBox("検品実績の取り消しを行いますか？(該当する品番全てが取り消されます。)", vbYesNo + vbDefaultButton2, "確認入力")
...
Dim SUMI_FLG    As Boolean
    ...
    '------------------------------- 出荷予定(ﾎｽﾄｲﾒｰｼﾞ)の処理 「ID毎の問合せ№解除」
    
    SUMI_FLG = False
    Call UniCode_Conv(K4_Y_SYU_H.ID_NO, Left(SYUKA(TDBGrid1.Bookmark, ColID_NO), 7))  ' ID№
    com = BtOpGetGreaterEqual
    Do
        ...
        Select Case sts
            Case BtNoErr
                If Left(SYUKA(TDBGrid1.Bookmark, ColID_NO), 7) <> Left(StrConv(Y_SYU_HREC.ID_NO, vbUnicode), 7) Then
                    Exit Do
                End If
            Case BtErrEOF
                Exit Do
                ...
                Select Case sts
                    Case BtNoErr
                        If Left(SYUKA(TDBGrid1.Bookmark, ColID_NO), 7) <> Left(StrConv(Y_SYU_HREC.ID_NO, vbUnicode), 7) Then
                            ...
                            sts = BTRV(BtOpUnlock, Y_SYU_H_POS, Y_SYU_HREC, Len(Y_SYU_HREC), K4_Y_SYU_H, Len(K4_Y_SYU_H), 4)
                            If sts <> BtNoErr Then
