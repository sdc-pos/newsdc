...
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   2016.01.07
Public Type MULTI_TANKA_TBL_tag
    HINBAN          As String * 20
    TANKA()         As Double
End Type

Public MULTI_TANKA_TBL()    As MULTI_TANKA_TBL_tag
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   2016.01.07
...
End If
        ...
        If Trim(StrConv(P_STOCK_REC.CODE, vbUnicode)) = "" And _
            ...
            'Clng --> Val 2016.01.08
            ZEN_ZAIKO = Val(StrConv(P_STOCK_REC.ZEN_ZAIKO_QTY, vbUnicode))
...
'    End If
    ...
    Call UniCode_Conv(tmpP_STOCK_REC.FILLER, StrConv(P_STOCK_REC.FILLER, vbUnicode))
