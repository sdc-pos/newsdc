...
Private Type USE_HINBAN_TBL_Tag             '2012.05.21
        USE_HINBAN      As String * 20      '2012.05.21
        USE_CNT         As Long             '2012.05.21
        SKIP_CNT        As Long             '2012.05.21
End Type                                    '2012.05.21


Private Const LAST_UPDATE_DAY$ = "[PLN0070] 2012.05.25 15:00"
...
Dim Tbl_F               As Boolean                      '2012.05.23
Dim USE_HINBAN_TBL()    As USE_HINBAN_TBL_Tag           '2012.05.23
...
'---------------------------------------------------------->>>>>    入荷集計分クリアー 2012.01.17   <<<<<
    ...
    Do
        ...
        Select Case sts
            Case BtNoErr
    Call LOG_OUT(LOG_F, Trim(StrConv(P_SHORDER_REC.HIN_GAI, vbUnicode)))
        ...
        Call UniCode_Conv(P_SHORDER_REC.UPD_FLG, "")
        sts = BTRV(BtOpUpdate, P_SHORDER_POS, P_SHORDER_REC, Len(P_SHORDER_REC), K5_P_SHORDER, Len(K5_P_SHORDER), 5)
        Select Case sts
            Case BtNoErr
...
'<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<指図票発行済みﾁｪｯｸ 2012.05.23>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    Erase USE_HINBAN_TBL
    Tbl_F = True
                                    
                                    
    Call UniCode_Conv(K3_P_SSHIJI_O.HAKKO_DT, Format(Start_Date, "YYYYMMDD"))
    ...
    Do
        ...
        Select Case sts
            Case BtNoErr
                ...
                If StrConv(P_SSHIJI_O_REC.HAKKO_DT, vbUnicode) > Format(Now, "YYYYMMDD") Then
                    Exit Do
                End If
            
                    
                    
                If StrConv(P_SSHIJI_O_REC.CANCEL_F, vbUnicode) = P_CANCEL_OFF Then
                    
                    
                    
                    If Tbl_F Then
                        ReDim USE_HINBAN_TBL(0 To 0)
                        USE_HINBAN_TBL(0).USE_HINBAN = StrConv(P_SSHIJI_O_REC.HIN_GAI, vbUnicode)
                        USE_HINBAN_TBL(0).USE_CNT = 1
                        USE_HINBAN_TBL(0).SKIP_CNT = 0
                        Tbl_F = False
                    Else
                        For i = 0 To UBound(USE_HINBAN_TBL)
                            If USE_HINBAN_TBL(i).USE_HINBAN = StrConv(P_SSHIJI_O_REC.HIN_GAI, vbUnicode) Then
                                USE_HINBAN_TBL(i).USE_CNT = USE_HINBAN_TBL(i).USE_CNT + 1
                                Exit For
                            End If
                        Next i
                        If i > UBound(USE_HINBAN_TBL) Then
                            ReDim Preserve USE_HINBAN_TBL(0 To i)
                            USE_HINBAN_TBL(i).USE_HINBAN = StrConv(P_SSHIJI_O_REC.HIN_GAI, vbUnicode)
                            USE_HINBAN_TBL(i).USE_CNT = 1
                            USE_HINBAN_TBL(i).SKIP_CNT = 0
                        End If
                    End If
            ...
            Case Else
                ...
                Call Input_UnLock
                Call File_Error(sts, com, "指図票ﾃﾞｰﾀ")
                Exit Function
        End Select
...
'<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<子部品展開>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    Do
        ...
        Else
            For i = 0 To UBound(USE_HINBAN_TBL)
                If USE_HINBAN_TBL(i).USE_HINBAN = StrConv(PLN_S_YOTEI_R.HIN_GAI, vbUnicode) Then
                    If USE_HINBAN_TBL(i).USE_CNT > USE_HINBAN_TBL(i).SKIP_CNT Then
                        USE_HINBAN_TBL(i).SKIP_CNT = USE_HINBAN_TBL(i).SKIP_CNT + 1
                        SKIP_FLG = True
                        Exit For
                    End If
                End If
...
'---------------------------------------------------------->>>>>    入荷集計   <<<<<
                ...
                Do
                    ...
                    If StrConv(P_SHORDER_REC.CANCEL_F, vbUnicode) = P_CANCEL_ON Then
                        SKIP_FLG = True
                    End If
If Trim(StrConv(P_SHORDER_REC.HIN_GAI, vbUnicode)) = "D304" Then
    Debug.Print
End If
                    '2012.01.17
                    If StrConv(P_SHORDER_REC.UPD_FLG, vbUnicode) = "1" Then
                        SKIP_FLG = True
                    End If
                    '2012.01.17
                    ...
                    If Not SKIP_FLG Then
                        ...
                        '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>2012.01.17
                        Call UniCode_Conv(P_SHORDER_REC.UPD_FLG, "1")
                        sts = BTRV(BtOpUpdate, P_SHORDER_POS, P_SHORDER_REC, Len(P_SHORDER_REC), K5_P_SHORDER, Len(K5_P_SHORDER), 5)
                        Select Case sts
                            Case BtNoErr
...
Dim TOTAL_NYUKA_QTY     As Long
    ...
    wkBackColor = False         '2012.05.25
    svBookMark = -1             '2012.05.25
        ...
        For i = 0 To UBound(Z_RIREKI_Tbl)
            ...
            If Check2.Value = vbChecked Then
                ...
                For j = colDAY To Max_Col
                    If CLng(Z_RIREKI_Tbl(i).ZAIKO_QTY(j - colDAY)) < 0 Then
                        SKIP_FLG = False
                        Exit For
                    End If
                Next j
            ...
            If Not SKIP_FLG Then    '2012.05.25
