VERSION 5.00
Begin VB.Form F1040201 
   BackColor       =   &H00FFFFFF&
   Caption         =   "品番別在庫一覧表印刷([F104020] 2012.10.11 09:00) "
   ClientHeight    =   6960
   ClientLeft      =   2025
   ClientTop       =   2550
   ClientWidth     =   11295
...
Private Const Last_Update_Day$ = "[F104020]2012.10.11 09:00"
...
'印刷中は「印刷中断」以外のイベント取得不可
    ...
    Call UniCode_Conv(K6_ZAIKO.NAIGAI, Right(Combo(pcmbNAIGAI).Text, 1))
    Call UniCode_Conv(K6_ZAIKO.HIN_GAI, Text(ptxS_HIN_GAI).Text)
    ...
    Do
        ...
        Select Case sts
            Case BtNoErr
                If StrConv(ZAIKOREC.JGYOBU, vbUnicode) <> Last_JGYOBU Or _
                    ...
                    RTrim(StrConv(ZAIKOREC.HIN_GAI, vbUnicode)) > Text(ptxE_HIN_GAI).Text Then
                    Exit Do
                End If
            Case BtErrEOF
                Exit Do
        ...
        End If
                                        'コントロールブレーク
        If Len(RTrim(SAVE_HIN_GAI)) = 0 Then
        ...
        If PRI_HIN_GAI <> StrConv(ZAIKOREC.HIN_GAI, vbUnicode) Then
            ...
            Select Case sts
                Case BtNoErr
                    ...
                    Printer.Print LeftB(StrConv(ITEMREC.HIN_NAME, vbUnicode), 44);
...
'2012.10.10    Ret = InStr(1, Trim(fileName), ".") - 1
    ...
    Write #FileNo, "品番（外）", "入荷日", "品番（内）", "棚番", " ", "在庫数", "累計数"
...
'----------------------------------------------------------------------------
    ...
    F1040201.MousePointer = vbHourglass

    Call Ctrl_Lock(F1040201)
...
'----------------------------------------------------------------------------
    ...
    Call Ctrl_UnLock(F1040201)


    F1040201.MousePointer = vbDefault
...
Dim sts As Integer
    ...
    For i = 0 To UBound(JGYOBU_T)
        ...
        If JGYOBU_T(i).CODE = Last_JGYOBU Then
            F1040201.Caption = "品番別在庫一覧表印刷（" + RTrim(JGYOBU_T(i).NAME) + ")" & Last_Update_Day
...
'            LabJIGYO.BorderStyle = 1
    ...
    End If
                                '印刷フォント設定
    With NormalFont
        .NAME = F1040201.FontName
        .Size = F1040201.FontSize
    End With
    Set Printer.Font = NormalFont
                                '画面初期設定
...
Dim i As Integer
    ...
    Next i
                                    '事業部切り替え
    F1040201.Caption = "品番別在庫一覧表印刷（" + RTrim(JGYOBU_T(Index).NAME) + ")" & Last_Update_Day
