...
Begin VB.Form SEK00301 
   Caption         =   "積水ハウス邸別注文ﾃﾞｰﾀ　照合"
   ClientHeight    =   10665
   ClientLeft      =   2025
   ClientTop       =   -3390
   ClientWidth     =   15210
...
'Private Const LAST_UPDATE_DAY$ = "[SEK0030] 2018.04.06 13:00"
Private Const LAST_UPDATE_DAY$ = "[SEK0030] 2018.04.06 16:15"
...
Dim com     As String
    ...
    If GetIni(App.EXEName, "Zaiko_Tanaban", App.EXEName, c) Then
        Beep
        MsgBox "在庫引落用棚番を指定して下さい[SEK0030] Zaiko_Tanaban="
        End
    End If
    ...
    If GetIni(App.EXEName, "YOIN", App.EXEName, c) Then
        Beep
        MsgBox "出庫要因を指定して下さい[SEK0030] YOIN="
        End
    End If
    YOIN_CODE = RTrim(c)
    ...
    If GetIni(App.EXEName, "MUKE_CODE", App.EXEName, c) Then
        Beep
        MsgBox "積水向け先ｺｰﾄﾞを指定して下さい[SEK0030] MUKE_CODE="
        End
    End If
    ...
    SEK00301.Caption = SEK00301.Caption & " " & LAST_UPDATE_DAY
...
If Right(Format(in_cnt, "000000"), 2) = "00" Then

    hStatusWnd = CreateStatusWindow(WS_VISIBLE Or WS_CHILD Or CCS_BOTTOM Or SBARS_SIZEGRIP, _
        "積水ハウス邸別注文ﾃﾞｰﾀ照合" & "(IN) 品番= " & StrConv(Y_SYU_HREC.HIN_NO, vbUnicode) & "(" & in_cnt & ")", Me.hwnd, 0)
     
    DoEvents

End If
Call LOG_OUT(LOG_F, "積水ハウス邸別注文ﾃﾞｰﾀ照合" & "(IN) 品番= " & StrConv(Y_SYU_HREC.HIN_NO, vbUnicode))
...
Call LOG_OUT(LOG_F, "積水ハウス邸別注文ﾃﾞｰﾀ照合" & "(NG) 品番= " & StrConv(Y_SYU_HREC.HIN_NO, vbUnicode))
...
Call LOG_OUT(LOG_F, "積水ハウス邸別注文ﾃﾞｰﾀ照合" & "(OK) 品番= " & StrConv(Y_SYU_HREC.HIN_NO, vbUnicode))
...
Dim i   As Integer
    ...
    SEK00301.MousePointer = vbHourglass

    Call Ctrl_Lock(SEK00301)
...
Dim i   As Integer
    ...
    Call Ctrl_UnLock(SEK00301)


    SEK00301.MousePointer = vbDefault
...
hStatusWnd = CreateStatusWindow(WS_VISIBLE Or WS_CHILD Or CCS_BOTTOM Or SBARS_SIZEGRIP, _
    ...
    For i = 1 To Y_Syuka_TEI.Count(1)
        ...
        Write #FileNo, Y_Syuka_TEI(i, colSEQ_NO),
