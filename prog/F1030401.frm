VERSION 5.00
Begin VB.Form F1030401 
   BackColor       =   &H00FFFFFF&
   Caption         =   "向け先別出庫表印刷(向け先＞標準棚番別)"
   ClientHeight    =   8355
   ClientLeft      =   2325
   ClientTop       =   2715
   ClientWidth     =   12870
...
Private Const ptx_DENPYO_No% = 7            '伝票番号
...
Private Const LAST_UPDATE_DAY$ = "(F103040) 2020.03.31 11:00 向け先プルダウン拡張"
...
Dim wkFDen_No   As String
Dim wkTDen_No   As String
    ...
    If com = BtOpGetGreaterEqual Then
        ...
        Call UniCode_Conv(K5_Y_SYU.KEY_MUKE_CODE, Left(Right(Combo(pcmbMUKE_Code).Text, 16), 8))
        Call UniCode_Conv(K5_Y_SYU.KEY_SS_CODE, Right(Combo(pcmbMUKE_Code).Text, 8))
        ...
        Do
            ...
            Select Case sts
                Case BtNoErr
                    ...
                    End If
                                                        '出荷予定日
                    If StrConv(Y_SYUREC.SYUKA_YMD, vbUnicode) < (Text(ptxS_DEN_DT_YY).Text & Text(ptxS_DEN_DT_MM).Text & Text(ptxS_DEN_DT_DD).Text) Or _
                       StrConv(Y_SYUREC.SYUKA_YMD, vbUnicode) > (Text(ptxE_DEN_DT_YY).Text & Text(ptxE_DEN_DT_MM).Text & Text(ptxE_DEN_DT_DD).Text) Then
                        If Right(Combo(pcmbPRINT_KBN).Text, 1) = Print_KBN_SIN Then
                            sts = BTRV(BtOpUnlock, Y_SYU_POS, Y_SYUREC, Len(Y_SYUREC), K5_Y_SYU, Len(K5_Y_SYU), 5)
                            If sts Then
                                Call File_Error(sts, BtOpUnlock, "出荷予定ファイル")
                    ...
                    Else
                        ...
                        wkFDen_No = Trim(StrConv(Y_SYUREC.DEN_NO, vbUnicode))
                        
                        If IsNumeric(wkFDen_No) Then
                            wkFDen_No = Trim(Format(CLng(wkFDen_No), "#0"))
                        End If
                        
                        wkTDen_No = Trim(Text(ptx_DENPYO_No).Text)
                        
                        If IsNumeric(wkTDen_No) Then
                            wkTDen_No = Trim(Format(CLng(wkTDen_No), "#0"))
                        End If
                        
                        
                        If Trim(Text(ptx_DENPYO_No).Text) = "" Or _
                          (Trim(Text(ptx_DENPYO_No).Text) <> "" And _
                           wkFDen_No = wkTDen_No) Then
'                           Trim(StrConv(Y_SYUREC.DEN_NO, vbUnicode)) = Trim(Text(ptx_DENPYO_No).Text)) Then
...
'----------------------------------------------------------------------------
    ...
    F1030401.MousePointer = vbHourglass

    Call Ctrl_Lock(F1030401)
...
'----------------------------------------------------------------------------
    ...
    Call Ctrl_UnLock(F1030401)


    F1030401.MousePointer = vbDefault
...
Dim Skip_Flg    As Boolean
    ...
    Do
        ...
        If NON_MUKE_FLG Then
            Skip_Flg = False
            For i = 0 To UBound(NON_MUKE_CODE)
                If StrConv(MTSREC.MUKE_CODE, vbUnicode) = NON_MUKE_CODE(i) Then
                    Skip_Flg = True
                    Exit For
                End If
...
'    PrintForm                  '2017.02.14
   Call Form_HCopy_Win7_NEW(Picture1, vbPRPSA4, vbPRORLandscape)     '2017.02.14
...
'        End If
        ...
        If JGYOBU_T(i).CODE = Last_JGYOBU Then
            F1030401.Caption = "向け先別出庫表印刷(向け先＞標準棚番別)(" + RTrim(JGYOBU_T(i).NAME) + ")" & LAST_UPDATE_DAY     '2017.02.14
...
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>    2017.02.14  SYS.INI --> F103040.INI
    ...
    NON_MUKE_FLG = False
    Do
        If GetIni(App.EXEName, "MUKE" & Format(i + 1, "00"), App.EXEName, c) Then
            Exit Do
        End If
...
'    End If
    ...
    Next i
                                    '事業部切り替え
'    F1030401.Caption = "向け先別出庫表印刷（" + RTrim(JGYOBU_T(Index).NAME) + "）"                     '2017.02.14
    F1030401.Caption = "向け先別出庫表印刷（" + RTrim(JGYOBU_T(Index).NAME) + "）" & LAST_UPDATE_DAY    '2017.02.14
