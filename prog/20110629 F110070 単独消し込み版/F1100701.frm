...
Begin VB.Form F1100701 
   BackColor       =   &H00C0C0C0&
   Caption         =   "出荷予定強制完了 [F110070]2011.06.29 15:30"
   ClientHeight    =   4704
   ClientLeft      =   1920
   ClientTop       =   2436
   ClientWidth     =   7320
...
Dim c As String * 128
    ...
    End If
                                ...
                                '削除対象日付算出
    If GetIni(App.EXEName, "CMPLT_DAY", "SYS", c) Then
        Beep
        MsgBox "日付の獲得に失敗しました。処理を中止して下さい。"
        Call LOG_OUT(LOG_F, "[SYS.INI] [SYSTEM] [CMPLT_DAY] READ ERROR")
        End
    End If
    ...
    End If
                                '処理モード取り込み
    If GetIni(App.EXEName, "AUTO", "SYS", c) Then
    ...
    If GetIni(App.EXEName, "MTS", "SYS", c) Then
        c = " "
    End If
...
'この時点でのファイル使用中は無限ループとする。キャンセルで異常終了
        ...
        If sts = BtNoErr Then
            ...
            If Limit_day >= StrConv(Y_SYUREC.SYUKA_YMD, vbUnicode) Then
    ...
    If OSAKA_PC Then
        ...
        Do
            Do
                DoEvents
                sts = BTRV(com + BtSNoWait, Y_SYU_H_POS, Y_SYU_HREC, Len(Y_SYU_HREC), K0_Y_SYU_H, Len(K0_Y_SYU_H), 0)
                Select Case sts
                    Case BtNoErr
                        Exit Do
                    Case BtErrFILE_INUSE, BtErrRECORD_INUSE
    ...
    'この時点でのファイル使用中は無限ループとする。キャンセルで異常終了
            ...
            If sts = BtNoErr Then
                ...
                If Limit_day >= StrConv(Y_SYU_HREC.SYUKA_YMD, vbUnicode) Then
                    Undo = False
                                '2009.08.24
                '-------------------------  積水対応    2011.06.29
                    If KENPIN_CHECK_H_PROC(Undo) Then
                        Exit Do
                    End If
                '-------------------------  積水対応    2011.06.29
                    If Undo Then
...
Dim sts As Integer
    ...
    If Not IsNumeric(StrConv(Y_SYUREC.KAN_YMD, vbUnicode)) Then
...
Private Function KENPIN_CHECK_H_PROC(Undo As Boolean) As Integer
'-----------------------------------------------------------------------------------
'
'   大阪ＰＣ向け　検品済み　＆　キャンセルのチェック
'
...
Dim sts As Integer
    ...
    KENPIN_CHECK_H_PROC = True
    
    
    If Not IsNumeric(StrConv(Y_SYU_HREC.KENPIN_NOW, vbUnicode)) Then
