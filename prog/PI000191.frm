...
Begin VB.Form PI000191 
   Caption         =   "商品化指図票発行 (PI00010 2013.11.13 08:00)"
   ClientHeight    =   9525
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15240
...
'2013.09.04 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                Shimuke_flg = False
                If svSHIMUKE_CODE <> Mid(Right(Combo1(pcmbSHIMUKE), 4), 1, 2) Then
...
Dim rpt             As New PI00019F1
Dim f               As New PI000193

Dim rpt2            As New PI00019F2
...
'                End If
        ...
        Case cmdNext                        '構成部品画面へ
            ...
            Doukon_Start = 1
            PI000192.Show vbModal           '部品詳細フォーム表示
...
'                                    ans = MsgBox("会社名/事業部 が指定されていません。(ＯＫ＝発行、ｷｬﾝｾﾙ=発行しない)", vbOKCancel + vbQuestion + vbDefaultButton2, "確認入力") '2013.08.29
...
' 2013.01.05 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                            ...
                            Else
                                ...
                                If Trim(txGensankoku.text) = "" Then
                                    ...
                                    ans = MsgBox("原産国が空白です。ラベル印刷しますか？", vbYesNo + vbQuestion + vbDefaultButton2, "確認入力")                    '2013.08.29
                                    'If ans = vbCancel Then '2013.08.29
...
' 2013.01.05 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                            ...
                            If KISHU_CHECK Then
                                ...
                                If Trim(KISHU1) = "" And Trim(KISHU2) = "" Then
                                    ...
                                    ans = MsgBox("代表機種が空白です。ラベル印刷しますか?", vbYesNo + vbQuestion + vbDefaultButton2, "確認入力")                   '2013.08.29
                                    If ans = vbYes Then
                                    Else
                                        L_print_Flg = False
                                    End If
...
''                                                                    0
                    ...
                    Else                                                '2008.02.02
                        ...
                        Set rpt2 = New PI00019F2                        '2008.02.02
                        'レポートを印刷します。（true：印刷ダイアログあり false：なし）
...
Dim sts As Integer
    ...
    End If
    Set PI000191 = Nothing
    Set PI000192 = Nothing
    Set PI000193 = Nothing
