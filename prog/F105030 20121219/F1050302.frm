...
Begin VB.Form F1050302 
   ...
   Begin TrueDBGrid80.TDBGrid TDBGrid1 
      ...
      _StyleDefs(30)  =   "Splits(0).Columns(0).Style:id=40,.parent=11,.alignment=3,.locked=-1"
      _StyleDefs(31)  =   "Splits(0).Columns(0).HeadingStyle:id=37,.parent=12,.alignment=3"
      _StyleDefs(32)  =   "Splits(0).Columns(0).FooterStyle:id=38,.parent=13,.alignment=3"
      _StyleDefs(33)  =   "Splits(0).Columns(0).EditorStyle:id=39,.parent=15"
      _StyleDefs(34)  =   "Splits(0).Columns(1).Style:id=24,.parent=11,.alignment=3,.locked=-1"
      _StyleDefs(35)  =   "Splits(0).Columns(1).HeadingStyle:id=21,.parent=12,.alignment=3"
      _StyleDefs(36)  =   "Splits(0).Columns(1).FooterStyle:id=22,.parent=13,.alignment=3"
      _StyleDefs(37)  =   "Splits(0).Columns(1).EditorStyle:id=23,.parent=15"
      _StyleDefs(38)  =   "Splits(0).Columns(2).Style:id=28,.parent=11,.alignment=2,.locked=0"
...
Private Const P_ID = "F105030"      'プログラムＩＤ．
...
Dim i As Integer
    ...
    F1050302.MousePointer = vbHourglass

    Call Ctrl_Lock(F1050302)
...
Dim i As Integer
    ...
    Call Ctrl_UnLock(F1050302)


    F1050302.MousePointer = vbDefault
...
Dim Soko_No     As String * 2
    ...
    Open Trim(GW_Path) For Output As FileNo
                                'ヘッダ出力 2012.10.16　出力元プログラム追加
    Write #FileNo, "事業部", "国内外", "品番(外部)", "品名", "要因", "特", "実績日付", "実績時刻", "伝票日付", "伝票№", "総在庫数", "入庫数(済)", "入庫数(未)", "出庫数(済)", "出庫数(未)", "在訂(済)", "在訂(未)", "移動(済)", "移動(未)", "入出庫元", "入出庫先", "入荷日", "向け先", "ID", "品番(内部)", "メモ", "伝票ＩＤ", "担当者CD", "担当者名", "仕入先", "仕入単価", "標準棚番", "出力元プログラム"
...
Debug.Print StrConv(IDOREC.RIRK_ID, vbUnicode)
            ...
            Write #FileNo, Trim(StrConv(IDOREC.SHIIRE_CODE, vbUnicode)),    '2007.05.02
            Write #FileNo, Trim(StrConv(IDOREC.SHIIRE_TANKA, vbUnicode)),    '2007.05.02
                    
                        
            
            Call UniCode_Conv(K0_ITEM.JGYOBU, StrConv(IDOREC.JGYOBU, vbUnicode))        '2008.04.02
            Call UniCode_Conv(K0_ITEM.NAIGAI, StrConv(IDOREC.NAIGAI, vbUnicode))        '2008.04.02
            Call UniCode_Conv(K0_ITEM.HIN_GAI, StrConv(IDOREC.HIN_GAI, vbUnicode))      '2008.04.02
            ...
            Else
                Write #FileNo, StrConv(ITEMREC.ST_SOKO, vbUnicode) & "-" & _
            ...
            Write #FileNo, Trim(StrConv(IDOREC.PRG_ID, vbUnicode)),         '2012.10.16
