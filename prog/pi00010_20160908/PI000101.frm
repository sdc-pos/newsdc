...
'Private Const Last_Update_day$ = "商品化指図票発行 (PI00010 2016.02.10 14:00)"
Private Const Last_Update_day$ = "商品化指図票発行 (PI00010 2016.02.10 15:00)"
...
Dim wkTITLE     As Variant      '2016.09.08
...
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 商品化検査欄 2016.09.08
    For i = 0 To 6
        If GetIni(App.EXEName, "CHECK_ITEM" & Format(i + 1, "00"), App.EXEName, c) Then
            CHECK_ITEM_TBL(0).TITLE = ""
            CHECK_ITEM_TBL(0).HANTEI = ""
        Else
            wkTITLE = Split(Trim(c), ",", -1)
            
            CHECK_ITEM_TBL(i).TITLE = wkTITLE(0)
            CHECK_ITEM_TBL(i).FONT_SIZE = wkTITLE(1)
            
            
            If UBound(wkTITLE) > 1 Then
                CHECK_ITEM_TBL(i).HANTEI = wkTITLE(2)
            Else
                CHECK_ITEM_TBL(i).HANTEI = ""
            End If
