VERSION 5.00
Begin {2AF752CD-B826-4828-B4C1-13BFC9CC90C1} PI00010F1 
   ClientHeight    =   14805
   ClientLeft      =   150
   ClientTop       =   570
   ClientWidth     =   19050
   ...
   _ExtentX        =   33602
   _ExtentY        =   26114
   SectionData     =   "PI00010F1.dsx":0000
End
Attribute VB_Name = "PI00010F1"
...
Dim c           As String * 128
    ...
    If PRINT_STOP_F Then            '2015.03.16
        Exit Sub                    '2015.03.16
    End If                          '2015.03.16
...
'                        eof = False                '2012.04.17
            ...
            Case Else
                        ...
                        If sts > 3000 Or sts = 3 Then
                            ...
                            Call File_Error(sts, BtOpGetEqual, "対象指図票ﾃﾞｰﾀ", 0)

                            MsgBox "印刷中にデータ読込み異常が発生しました。再印刷を実行して下さい。"
    ...
    Select Case sts
        ...
        Case Else
            ...
            If sts > 3000 Or sts = 3 Then
                ...
                Call File_Error(sts, BtOpGetEqual, "指図票ﾃﾞｰﾀ", 0)
    ...
    Select Case sts
        ...
        Case Else
                        ...
                        If sts > 3000 Or sts = 3 Then
                            ...
                            MsgBox "印刷中にデータ読込み異常が発生しました。再印刷指示して下さい。"
...
'    lblSagyo_day3.Visible = PRI_SAGYO_DAY
    ...
    Do
        ...
        If StrConv(GENSANREC.UPD_DATETIME, vbUnicode) > StrConv(GENSANREC.Ins_DateTime, vbUnicode) Then
            PI000101.lstGensankoku.AddItem StrConv(GENSANREC.UPD_DATETIME, vbUnicode) & StrConv(GENSANREC.GENSANKOKU, vbUnicode)
        Else
            PI000101.lstGensankoku.AddItem StrConv(GENSANREC.Ins_DateTime, vbUnicode) & StrConv(GENSANREC.GENSANKOKU, vbUnicode)
        End If
    ...
    fieldGensan01.Visible = False
    fieldGensan02.Visible = False
    fieldGensan03.Visible = False
    
    j = 0
    
    
    If PI000101.lstGensankoku.ListCount > 0 Then
        For i = PI000101.lstGensankoku.ListCount - 1 To 0 Step -1
            ...
            Select Case j
                Case 1
                    fieldGensan01.text = Right(PI000101.lstGensankoku.List(i), 20)
                    fieldGensan01.Visible = True
                Case 2
                    fieldGensan02.text = Right(PI000101.lstGensankoku.List(i), 20)
                    fieldGensan02.Visible = True
                Case 3
                    fieldGensan03.text = Right(PI000101.lstGensankoku.List(i), 20)
                    fieldGensan03.Visible = True
            End Select
...
'    '同梱部品   '2011.08.04
    ...
    lblITEM01.Caption = CHECK_ITEM_TBL(0).TITLE
    lblITEM01.Font.Size = CHECK_ITEM_TBL(0).FONT_SIZE
    lblHANTEI01.Caption = CHECK_ITEM_TBL(0).HANTEI


    lblITEM02.Caption = CHECK_ITEM_TBL(1).TITLE
    lblITEM02.Font.Size = CHECK_ITEM_TBL(1).FONT_SIZE
    lblHANTEI02.Caption = CHECK_ITEM_TBL(1).HANTEI

    lblITEM03.Caption = CHECK_ITEM_TBL(2).TITLE
    lblITEM03.Font.Size = CHECK_ITEM_TBL(2).FONT_SIZE
    lblHANTEI03.Caption = CHECK_ITEM_TBL(2).HANTEI

    lblITEM04.Caption = CHECK_ITEM_TBL(3).TITLE
    lblITEM04.Font.Size = CHECK_ITEM_TBL(3).FONT_SIZE
    lblHANTEI04.Caption = CHECK_ITEM_TBL(3).HANTEI

    lblITEM05.Caption = CHECK_ITEM_TBL(4).TITLE
    lblITEM05.Font.Size = CHECK_ITEM_TBL(4).FONT_SIZE
    lblHANTEI05.Caption = CHECK_ITEM_TBL(4).HANTEI

    lblITEM06.Caption = CHECK_ITEM_TBL(5).TITLE
    lblITEM06.Font.Size = CHECK_ITEM_TBL(5).FONT_SIZE
    lblHANTEI06.Caption = CHECK_ITEM_TBL(5).HANTEI

    lblITEM07.Caption = CHECK_ITEM_TBL(6).TITLE
    lblITEM07.Font.Size = CHECK_ITEM_TBL(6).FONT_SIZE
    lblHANTEI07.Caption = CHECK_ITEM_TBL(6).HANTEI
            ...
            Select Case sts
                ...
                Case Else
                    ...
                    If sts > 3000 Or sts = 3 Then
                        ...
                        Call File_Error(sts, BtOpGetEqual, "入出庫単価設定マスタ", 0)
