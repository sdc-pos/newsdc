VERSION 5.00
Begin VB.Form CONV_ITEM_201209241 
   BackColor       =   &H00C0C0C0&
   Caption         =   "データコンバート処理（CONV_ITEM_20120924 2012.09.24 10:00)"
   ClientHeight    =   7230
   ClientLeft      =   2325
   ClientTop       =   2625
   ClientWidth     =   10095
   ...
   End
End
Attribute VB_Name = "CONV_ITEM_201209241"
...
'---------------------------------------------  受入履歴データのコンバート
    ...
    Do
        ...
        If StrConv(ITEMREC.S_KOUSU_SET_DATE, vbUnicode) < "20120223" Then
        Else
            If Trim(StrConv(ITEMREC.S_KOUSU, vbUnicode)) = "" Then
            
                
Debug.Print StrConv(ITEMREC.S_KOUSU_SET_DATE, vbUnicode)
                If IsNumeric(StrConv(ITEMREC.CATE_AD_FUN, vbUnicode)) Then
                    
                    sel_count = sel_count + 1
                    Cnt(1).Caption = Format(sel_count, "#0")
                    
                    
                    Call UniCode_Conv(ITEMREC.BEF_S_KOUSU, StrConv(ITEMREC.S_KOUSU, vbUnicode))
                    Call UniCode_Conv(ITEMREC.S_KOUSU, StrConv(ITEMREC.CATE_AD_FUN, vbUnicode))
            
                    Call UniCode_Conv(ITEMREC.UPD_DATETIME, Start_Now)
                    ...
                    Call LOG_OUT(LOG_F, StrConv(ITEMREC.JGYOBU, vbUnicode) & " " & StrConv(ITEMREC.NAIGAI, vbUnicode) & " " & StrConv(ITEMREC.HIN_GAI, vbUnicode))
...
Dim sts As Integer
    ...
    Set CONV_ITEM_201209241 = Nothing
