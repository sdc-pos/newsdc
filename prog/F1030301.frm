...
Private Const ptx_DENPYO_No% = 8            '伝票番号
...
Private DEF_CYU_KBN  As String * 1          'ﾃﾞﾌｫﾙﾄ注文区分　2012.10.01
...
Private Const LAST_UPDATE_DAY$ = "[F103030]2012.11.28 17:00"
...
Dim i   As Integer
    ...
    For i = 0 To UBound(JGYOBU_T) - 1
        ...
        If JGYOBU_T(i).CODE = Last_JGYOBU Then
            F1030301.Caption = "標準棚番別出庫表印刷（" + RTrim(JGYOBU_T(i).NAME) + ") " & LAST_UPDATE_DAY
...
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>    専用INIに移行   2012.10.01
    ...
    If GetIni(App.EXEName, "DEF_CYU_KBN", App.EXEName, c) Then
        DEF_CYU_KBN = CYU_KBN_SPO
    Else
        DEF_CYU_KBN = Trim(c)
    End If
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>    専用INIに移行   2012.10.01
...
'    Combo(pcmbCyu_Kbn).AddItem CYU_KBN_0 & "   " & CYU_KBN_HSP
    ...
    For i = 0 To Combo(pcmbCyu_Kbn).ListCount - 1
        If DEF_CYU_KBN = Right(Combo(pcmbCyu_Kbn).List(i), 1) Then
...
'    End If
    ...
    Next i
                                    '事業部切り替え
    F1030301.Caption = "標準棚番別出庫表印刷（" + RTrim(JGYOBU_T(Index).NAME) + ") " & LAST_UPDATE_DAY
