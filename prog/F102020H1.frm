...
Private Type YUKO_SOKO_TBL4             '有効ﾎｽﾄ倉庫取り込みテーブル（炊飯機器事業部）
    ...
    NAIGAI As String * 1
End Type
Dim SOKO_T4(ZERO To 9) As YUKO_SOKO_TBL4

Private Type YUKO_SOKO_TBLD             '有効ﾎｽﾄ倉庫取り込みテーブル（電化調理機器事業部）
    HS_SOKO As String * 2
    NAIGAI As String * 1
End Type
Dim SOKO_TD(ZERO To 9) As YUKO_SOKO_TBLD

Private Type YUKO_SOKO_TBL1             '有効ﾎｽﾄ倉庫取り込みテーブル（ｱｲﾛﾝ事業部）
    HS_SOKO As String * 2
    NAIGAI As String * 1
End Type
Dim SOKO_T1(ZERO To 9) As YUKO_SOKO_TBL1

Dim JIGYOBU_BEF As String * 1           'ｱｲﾛﾝ元事業部       2001.11.01
Dim JIGYOBU_AFT As String * 1           'ｱｲﾛﾝ先事業部       2001.11.01
Dim DEN_NO      As String * 1           'ｱｲﾛﾝ伝票№特殊区分 2001.11.01

Dim WS_NO As String * 2                 'ﾜｰｸｽﾃｰｼｮﾝ番号

Dim HS_NaiG As String                   '国内外（決定内容）････　ﾎｽﾄﾃﾞｰﾀ内容により設定
Dim BEF_GAI As String * 13              '変更前品番（外部）
Dim BEF_NAI As String * 13              '変更前品番（内部）
...
Dim Disp_Index As Integer

Private Type JGYOBU_Cnt_tbl_tag
    Code    As String * 1
    Out_Cnt As Integer
End Type


Dim JGYOBU_Cnt_tbl() As JGYOBU_Cnt_tbl_tag
'''''''''''''''''''''''''''''''''''''
                                    '画面初期表示（処理済「便」表示など）
...
'ホストデータ　チェック＆取込み
    ...
    Do
        ...
        If sts = False Then
            ...
            Else
                Call Proc_Sel                       '売上(0)，入荷(1)ﾃﾞｰﾀ更新
            End If
        End If
    Loop
...
'重要　※　特売りは４便とみなす　※
    If Shori_Mode > ZERO Then
                                        '炊飯機器事業部  ホスト受信データ取込み
        If HS_SIJ_Open4(ZERO, Format(Shori_Mode, "0")) = False Then
                                                        'ﾌｧｲﾙ無しなら処理しない
            Close #HS_SIJ_No
                                                        '炊飯機器事業部ﾎｽﾄﾃﾞｰﾀ OPEN
            If HS_SIJ_Open4(1, Format(Shori_Mode, "0")) Then
                Unload Me
            End If
        ...
        End If
                                        '電化調理機器事業部  ホスト受信データ取込み
        Disp_Index = 1
        If HS_SIJ_OpenD(ZERO, Format(Shori_Mode, "0")) = False Then
                                                        'ﾌｧｲﾙ無しなら処理しない
            Close #HS_SIJ_No
                                                        '電化調理機器事業部ﾎｽﾄﾃﾞｰﾀ OPEN
            If HS_SIJ_OpenD(1, Format(Shori_Mode, "0")) Then
                Unload Me
            End If
...
Dim In_Cnt  As Integer
    ...
    Do
        ...
        labIn_Cnt(Disp_Index).Caption = StrConv(Format(In_Cnt, "#0"), vbWide)
        ...
        Put_Sel = True
                                                '事業部区分　範囲外？
        If StrConv(HS_SIJREC.JGYOBU, vbUnicode) = JIGYOBU_BEF Then '（洗濯機（ｱｲﾛﾝ）だったら無条件でＯＫ） 2001.11.01
        Else
        ...
        End If
                                                'ホスト倉庫　範囲外？
        Select Case StrConv(HS_SIJREC.JGYOBU, vbUnicode)
            Case SUIHAN
                For i = ZERO To UBound(SOKO_T4) - 1
                    If SOKO_T4(i).HS_SOKO = "  " Then
                        ...
                        Exit For
                    End If
                    If RTrim(StrConv(HS_SIJREC.HOST_SOKO, vbUnicode)) = RTrim(SOKO_T4(i).HS_SOKO) Then
                        Exit For
                    End If
                Next i
            Case DENKA
                For i = ZERO To UBound(SOKO_TD) - 1
                    If SOKO_TD(i).HS_SOKO = "  " Then
                        ...
                        Exit For
                    End If
                    If RTrim(StrConv(HS_SIJREC.HOST_SOKO, vbUnicode)) = RTrim(SOKO_TD(i).HS_SOKO) Then
                        Exit For
                    End If
                Next i
            Case JIGYOBU_BEF
...
'国内外区分の設定
    ...
    Select Case StrConv(XX_SIJREC.JGYOBU, vbUnicode)
        Case SUIHAN
            For i = ZERO To UBound(SOKO_T4)
                If SOKO_T4(i).HS_SOKO = "  " Then
                    Exit For
                End If
                If RTrim(StrConv(XX_SIJREC.HOST_SOKO, vbUnicode)) = RTrim(SOKO_T4(i).HS_SOKO) Then
        ...
        Case DENKA
            For i = ZERO To UBound(SOKO_TD)
                If SOKO_TD(i).HS_SOKO = "  " Then
                    Exit For
                End If
                If RTrim(StrConv(XX_SIJREC.HOST_SOKO, vbUnicode)) = RTrim(SOKO_TD(i).HS_SOKO) Then
...
'同一出荷伝票存在チェック(事業部＋注文区分＋伝票№)
                                                    '事業部
        If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = JIGYOBU_BEF Then
                                                    '事業部読替え 2001.11.01
            Call UniCode_Conv(K0_Y_SYU.JGYOBU, JIGYOBU_AFT)
        Else
        ...
        If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = JIGYOBU_BEF Then
                                                    '伝票№読替え2001.11.01
            Call UniCode_Conv(K0_Y_SYU.DEN_NO, DEN_NO & Right(StrConv(XX_SIJREC.DEN_NO, vbUnicode), 5))
        Else
...
'重複チェック用データ更新（再処理データ（終端＝「？」「＊」は更新しない）
    If StrConv(XX_SIJREC.REC_END, vbUnicode) <> "?" And StrConv(XX_SIJREC.REC_END, vbUnicode) <> "*" Then
        ...
        Call UniCode_Conv(SEQCKREC.LAST_GET_TM, Format(Time, "hhmmss"))                 '最終取込み時刻
...
'品番マスタデータ（外部）有無チェック
    If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = JIGYOBU_BEF Then
                                        '事業部読替え2001.11.01
        Call UniCode_Conv(K0_ITEM.JGYOBU, JIGYOBU_AFT)
    Else
...
'品番マスタデータ（内部）有無チェック
    If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = JIGYOBU_BEF Then
                                        '事業部読替え2001.11.01
        Call UniCode_Conv(K3_ITEM.JGYOBU, JIGYOBU_AFT)
    Else
...
'品目マスタ　外部品番変更
    ...
    If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = JIGYOBU_BEF Then
                            '事業部読替え2001.11.01
        Call UniCode_Conv(ITEMREC.JGYOBU, JIGYOBU_AFT)
    Else
...
Dim Work As String
    ...
    End If
                                ...
                                '事業部読替え2001.11.01
    If Command = BtOpInsert Then
        
                                '新規登録時は、無条件で該当事業部
        Call UniCode_Conv(ITEMREC.MOTO_JIGYOBU, StrConv(XX_SIJREC.JGYOBU, vbUnicode))
    Else
        If StrConv(ITEMREC.MOTO_JIGYOBU, vbUnicode) = SUIHAN Then
        Else
            If StrConv(ITEMREC.MOTO_JIGYOBU, vbUnicode) = JIGYOBU_BEF Then
                If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = SUIHAN Then
                                'アイロンで登録済でも炊飯で来たら炊飯に置き換え
                    Call UniCode_Conv(ITEMREC.MOTO_JIGYOBU, StrConv(XX_SIJREC.JGYOBU, vbUnicode))
                End If
            End If
            If Len(Trim(StrConv(ITEMREC.MOTO_JIGYOBU, vbUnicode))) = ZERO Then
                                '既存データで
                Select Case StrConv(XX_SIJREC.JGYOBU, vbUnicode)
                    Case DENKA
                                '電化で来た場合は電化
                    ...
                    Case JIGYOBU_BEF
                                'アイロンで来た場合は炊飯
                        Call UniCode_Conv(ITEMREC.MOTO_JIGYOBU, JIGYOBU_AFT)
                End Select
            End If
        End If
...
Dim WK_JIGYOBU  As String * 1   '2001.11.01
...
Dim i       As Integer
    ...
    NyukaY_Put = True
                            ...
                            '事業部読替え2001.11.01
    If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = JIGYOBU_BEF Then
        WK_JIGYOBU = JIGYOBU_AFT
    Else
        WK_JIGYOBU = StrConv(XX_SIJREC.JGYOBU, vbUnicode)
    End If
    
    For i = ZERO To UBound(JGYOBU_Cnt_tbl)
        If JGYOBU_Cnt_tbl(i).Code = StrConv(XX_SIJREC.JGYOBU, vbUnicode) Then
            JGYOBU_Cnt_tbl(i).Out_Cnt = JGYOBU_Cnt_tbl(i).Out_Cnt + 1
        End If
        labOut_Cnt(i).Caption = StrConv(Format(JGYOBU_Cnt_tbl(i).Out_Cnt, "#0"), vbWide)
    
    Next i
    
    
    Call UniCode_Conv(K4_Y_NYU.JGYOBU, WK_JIGYOBU)
...
'1997.08.22
    ...
    If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = "D" Then
        If RTrim(StrConv(XX_SIJREC.YOSAN_FROM, vbUnicode)) = "444" And _
            RTrim(StrConv(XX_SIJREC.HOST_SOKO, vbUnicode)) = "1" Then
            Call UniCode_Conv(XX_SIJREC.CYOK_KBN, "*")          '排除データは「直送区分」に「＊」をセット
        End If
        
        If RTrim(StrConv(XX_SIJREC.YOSAN_FROM, vbUnicode)) = "NC" And _
            RTrim(StrConv(XX_SIJREC.HOST_SOKO, vbUnicode)) = "1" Then
            Call UniCode_Conv(XX_SIJREC.CYOK_KBN, "*")          '排除データは「直送区分」に「＊」をセット
        End If
...
'    Call UniCode_Conv(Y_NYUREC.JGYOBU, StrConv(XX_SIJREC.JGYOBU, vbUnicode))
    Call UniCode_Conv(Y_NYUREC.JGYOBU, WK_JIGYOBU)
                                'ﾃｷｽﾄ№
...
'    Call UniCode_Conv(K0_J_NYU.JGYOBU, StrConv(XX_SIJREC.JGYOBU, vbUnicode))
                                    '2001.11.01
    Call UniCode_Conv(K0_J_NYU.JGYOBU, WK_JIGYOBU)
...
'入荷数で在庫データ更新（＋）2001.11.01
    If Nyuko_Update_Proc(WK_JIGYOBU, _
                            HS_NaiG, _
...
'在庫データLOCK     2001.11.01
        ...
        End If
                ...
                '2001.11.01
        If Syuko_Update_Proc(WK_JIGYOBU, _
                                HS_NaiG, _
                                StrConv(XX_SIJREC.HIN_GAI, vbUnicode), _
                                StrConv(XX_SIJREC.DEN_DT, vbUnicode), _
                                (KASO_NYUKA_Soko & "01" & "01" & "01"), _
...
Dim i       As Integer
    ...
    If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = JIGYOBU_BEF Then
                                '事業部読替え2001.11.01
        Call UniCode_Conv(Y_SYUREC.JGYOBU, JIGYOBU_AFT)
    Else
    ...
    If StrConv(XX_SIJREC.JGYOBU, vbUnicode) = JIGYOBU_BEF Then
                                '伝票№読替え2001.11.01
        If Len(Trim(DEN_NO)) = ZERO Then
            Call UniCode_Conv(Y_SYUREC.DEN_NO, StrConv(XX_SIJREC.DEN_NO, vbUnicode))
        Else
            Call UniCode_Conv(Y_SYUREC.DEN_NO, DEN_NO & Right(StrConv(XX_SIJREC.DEN_NO, vbUnicode), 5))
        End If
    Else
        Call UniCode_Conv(Y_SYUREC.DEN_NO, StrConv(XX_SIJREC.DEN_NO, vbUnicode))
    End If
        ...
        Do
            ...
            Select Case sts
                ...
                Case BtErrRECORD_INUSE, BtErrFILE_INUSE
                    ...
                    End If
                Case Else
                    Call File_Error(sts, Command + BtSNoWait, "入荷ﾁｪｯｸﾃﾞｰﾀ")
                    Exit Sub
            End Select
        Loop
...
Dim com     As String
    ...
    Do
        ...
        ReDim Preserve JGYOBU_Cnt_tbl(i)
        JGYOBU_Cnt_tbl(i).Code = RTrim(c)
        i = i + 1
    ...
    JGYOBU_Cnt_tbl(i).Code = RTrim(c)
                                ...
                                'ｱｲﾛﾝ事業部先取り込み
    If GetIni("SENTAKU", "JIGYOBU_AFT", "SYS", c) Then
        Beep
        MsgBox "ｱｲﾛﾝ事業部先の獲得に失敗しました。処理を中止して下さい。"
        End
    End If
    JIGYOBU_AFT = RTrim(c)
    ...
    DEN_NO = ""
                                'ｱｲﾛﾝ事業部伝票№読替え
    If GetIni("SENTAKU", "DEN_NO", "SYS", c) Then
    Else
        DEN_NO = RTrim(c)
    End If
    ...
    i = ZERO
    Do
        If GetIni("NYUSYU_OK_SOKO", "SOKO4" & RTrim(Format(i + 1, "#0")), "SYS", c) Then
            Beep
        ...
        If GetIni("NYUSYU_OK_SOKO", "NAIG4" & RTrim(Format(i + 1, "#0")), "SYS", c) Then
            Beep
            MsgBox "システム異常が発生しました。処理を中止して下さい。"
    ...
    i = ZERO
    Do
        If GetIni("NYUSYU_OK_SOKO", "SOKOD" & RTrim(Format(i + 1, "#0")), "SYS", c) Then
            Beep
            MsgBox "システム異常が発生しました。処理を中止して下さい。"
            Call Log_Out(LOG_F, "[SYS.INI] [NYUSYU_OK_SOKO] [SOKO] READ ERROR")
            End
        ...
        If GetIni("NYUSYU_OK_SOKO", "NAIGD" & RTrim(Format(i + 1, "#0")), "SYS", c) Then
            Beep
            MsgBox "システム異常が発生しました。処理を中止して下さい。"
            Call Log_Out(LOG_F, "[SYS.INI] [NYUSYU_OK_SOKO] [NAIG] READ ERROR")
            End
    ...
    End If
                                '印刷フォント設定
    With NormalFont
        .NAME = F1020201.FontName
        .Size = F1020201.FontSize
    End With
...
Private Sub Labe_Click(Index As Integer)
...
Dim i   As Integer
    ...
    If Text1(ZERO).Visible Then
        If IsNumeric(Text1(ZERO).Text) And _
            IsNumeric(Text1(2).Text) And _
            IsNumeric(Text1(4).Text) Then
        Else
            If Not IsNumeric(Text1(ZERO).Text) Then
                Text1(ZERO).SetFocus
            End If
            ...
            Beep
            MsgBox "入力した項目はエラーです。数値を入力して下さい。"
            Exit Sub
        End If
    End If
...
'----------------------------------------------------------------------------
    ...
    F1020201.MousePointer = vbHourglass

    Call Ctrl_Lock(F1020201)
...
'----------------------------------------------------------------------------
    ...
    Call Ctrl_UnLock(F1020201)

    F1020201.MousePointer = vbDefault
...
'強制再実行処理
    ...
    Text1(4).Visible = True
                                                ...
                                                '炊飯 SPIC
    Call UniCode_Conv(K0_SEQCK.JGYOBU, "4")
    Call UniCode_Conv(K0_SEQCK.SEQ_MODE, "1")
    ...
    Text1(1).Text = StrConv(SEQCKREC.LAST_TXTNO, vbUnicode)
                                                ...
                                                '電化 SPIC
    Call UniCode_Conv(K0_SEQCK.JGYOBU, "D")
    Call UniCode_Conv(K0_SEQCK.SEQ_MODE, "1")
    ...
    Text1(2).Text = StrConv(SEQCKREC.LAST_TXTNO, vbUnicode)
    ...
    Text1(3).Text = StrConv(SEQCKREC.LAST_TXTNO, vbUnicode)

                                                'ｱｲﾛﾝ SPIC
    Call UniCode_Conv(K0_SEQCK.JGYOBU, JIGYOBU_BEF)
    Call UniCode_Conv(K0_SEQCK.SEQ_MODE, "1")
    ...
    Text1(4).Text = StrConv(SEQCKREC.LAST_TXTNO, vbUnicode)
    ...
    Text1(5).Text = StrConv(SEQCKREC.LAST_TXTNO, vbUnicode)
...
Dim ans As Integer
    ...
    If com = BtOpInsert Then
        Call UniCode_Conv(SEQCKREC.JGYOBU, "4")
        Call UniCode_Conv(SEQCKREC.SEQ_MODE, "1")
    End If
    ...
    If com = BtOpInsert Then
        Call UniCode_Conv(SEQCKREC.JGYOBU, "D")
        Call UniCode_Conv(SEQCKREC.SEQ_MODE, "1")
    End If

    Call UniCode_Conv(SEQCKREC.LAST_TXTNO, Text1(2).Text)
    Call UniCode_Conv(SEQCKREC.LAST_GET_DT, Format(Date, "yyyymmdd"))
    Call UniCode_Conv(SEQCKREC.LAST_GET_TM, Format(Time, "HHmmss"))
    ...
    Call UniCode_Conv(SEQCKREC.LAST_TXTNO, Text1(3).Text)
    Call UniCode_Conv(SEQCKREC.LAST_GET_DT, Format(Date, "yyyymmdd"))
    Call UniCode_Conv(SEQCKREC.LAST_GET_TM, Format(Time, "HHmmss"))
    ...
    If com = BtOpInsert Then
        Call UniCode_Conv(SEQCKREC.JGYOBU, JIGYOBU_BEF)
        Call UniCode_Conv(SEQCKREC.SEQ_MODE, "1")
    End If

    Call UniCode_Conv(SEQCKREC.LAST_TXTNO, Text1(4).Text)
    Call UniCode_Conv(SEQCKREC.LAST_GET_DT, Format(Date, "yyyymmdd"))
    Call UniCode_Conv(SEQCKREC.LAST_GET_TM, Format(Time, "HHmmss"))
    ...
    Call UniCode_Conv(SEQCKREC.LAST_TXTNO, Text1(5).Text)
    Call UniCode_Conv(SEQCKREC.LAST_GET_DT, Format(Date, "yyyymmdd"))
    Call UniCode_Conv(SEQCKREC.LAST_GET_TM, Format(Time, "HHmmss"))
