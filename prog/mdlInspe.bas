            ...
            For i = 0 To M_Gyo - 1
                ...
                Select Case i
                    ...
                    Case 2     '品番
                        ...
                        If IsNumeric(Trim(Hinban)) And (Len(Trim(Hinban)) <= 5) Then
                            
                            If Val(Hinban) = ID_KANRI_TBL(ING_No).SYUKA_QTY Then
                            ...
                            Else
                                ...
                                Call Err_Send_Proc(ID_KANRI_TBL(ING_No).YOIN_LONG_DNAME, ID_KANRI_TBL(ING_No).ID_NO, "出荷数エラー", Hinban, "")  '2017.09.22
                                Sendbuf = Text_Create_Proc()
                                ID_KANRI_TBL(ING_No).Step = ID_KANRI_TBL(ING_No).Step - 1
                                New_Inspe_Proc = False
                                Exit Function
                        ...
                        Else
                            ...
                            ID_KANRI_TBL(ING_No).Send_Text.Box_Type(3).Box_Type = TYPE_REF
                                                                                    '表示内容
                            Call UniCode_Conv(Send_Text.Box_Type(3).LCD, "出荷数：" & StrConv(Format(ID_KANRI_TBL(ING_No).SYUKA_QTY, "#0"), vbWide) & "-" & Format(ID_KANRI_TBL(ING_No).ITEM_READ_CNT, "#0"))
                            Call UniCode_Conv(ID_KANRI_TBL(ING_No).Send_Text.Box_Type(3).LCD, "出荷数：" & StrConv(Format(ID_KANRI_TBL(ING_No).SYUKA_QTY, "#0"), vbWide) & "-" & Format(ID_KANRI_TBL(ING_No).ITEM_READ_CNT, "#0"))
                                                                                    '数値初期表示
                            Send_Text.Box_Type(3).INIT = ""
                            ID_KANRI_TBL(ING_No).Send_Text.Box_Type(3).INIT = ""
                                                                                    '初期カーソル位置
