...
'                    ID_KANRI_TBL(ING_No).Step = Step_MENU1_REQ         2018.05.10
'                    ID_KANRI_TBL(ING_No).MENU_LV1 = ""                 2018.05.10
            
            
                        ID_KANRI_TBL(ING_No).Step = Step_TANTO_REQ          '2018.05.10
                        ID_KANRI_TBL(ING_No).JGYOBU = ""                    '2018.05.10
                                                                            
                        ID_KANRI_TBL(ING_No).MENU_LV1 = ""                  '2018.05.10
                        ID_KANRI_TBL(ING_No).MENU_LV2 = ""                  '2018.05.10
                               ...
                               ((StrConv(P_MENUREC.SAGYO(i).PARAM, vbUnicode) = (ID_KANRI_TBL(ING_No).MTS_CODE & ID_KANRI_TBL(ING_No).SS_CODE)) Or _
                                ...
                                Select Case sts
                                    ...
                                    Case BtNoErr
                                        ...
                                        ID_KANRI_TBL(ING_No).PARAM = RTrim(StrConv(P_MENUREC.SAGYO(i).PARAM, vbUnicode))
                                        
                                        ID_KANRI_TBL(ING_No).SAGYO_LOG = StrConv(P_MENUREC.SAGYO(i).LOG_OUT, vbUnicode)
'                                        ID_KANRI_TBL(ING_No).Tanaban
...
'2006.01.30                                            ID_KANRI_TBL(ING_No).SS_CODE = Right(StrConv(P_MENUREC.SAGYO(i).PARAM, vbUnicode), 8)
                                        ...
                                        '>>>>>>>>>>>>>> 2018.11.05
                                                                                            ...
                                                                                            '新検品（向け先指定）なら                                                                              検品（向け先指定）なら
                                        If StrConv(YOINREC.CODE_TYPE, vbUnicode) = ACT_NEW_KENPIN_MTS Then
                                            ID_KANRI_TBL(ING_No).CYU_KBN = StrConv(YOINREC.YOIN_CODE, vbUnicode)
                                        End If
                                    ...
                                    Case BtErrKeyNotFound
                                        ...
                                        Sendbuf = Text_Create_Proc()
                                        'ID_KANRI_TBL(ING_No).Step = Step_TANTO_REQ                     '2018.05.10
                                        ID_KANRI_TBL(ING_No).Step = ID_KANRI_TBL(ING_No).Step - 1       '2018.05.10
...
'-----------------------------------------  2012.03.06
    ...
    '>>>>>>>>>> 2018.11.05
        ...
        Case ACT_NEW_KENPIN             '検品
        
            If New_Inspe_Proc(Sendbuf, i, j) Then
                Exit Function
            End If
    
    
        Case ACT_NEW_KENPIN_MTS             '検品（ＭＴＳ読み込みあり）
        
            If New_Inspe_Proc_MTS(Sendbuf, i, j) Then
                Exit Function
            End If
...
'                Case Wel_sagyoin         '2020/04/06 作業時間要因
'
'                    If sagyo_Proc(Sendbuf, i, j) Then
'                        Exit Function
'                    End If
...
'            End If
        ...
        Case ACT_SHOUHINKA              '商品化ﾁｪｯｸ     2010.09.03
            Select Case ID_KANRI_TBL(ING_No).Sagyo_Code.CODE_TYPE & ID_KANRI_TBL(ING_No).Sagyo_Code.YOIN_CODE
                ...
                Case Wel_SHIJI_END          '商品化完了登録   2018.04.26
                
                    If SHIJI_END_PROC(Sendbuf, i, j) Then
                        Exit Function
                    End If
...
'-----------------------------------------------    モジュール 2014.06.24
        Case ACT_MODULE
            Select Case ID_KANRI_TBL(ING_No).Sagyo_Code.CODE_TYPE & ID_KANRI_TBL(ING_No).Sagyo_Code.YOIN_CODE
                ...
                Case Wel_MODULE_IN                                      '2018.10.03-->
                    If Module_In_Proc(Sendbuf, i, j) Then               '2018.10.03-->
                        Exit Function                                   '2018.10.03-->
                    End If                                              '2018.10.03-->
...
'-----------------------------------------------    (新)直送検品    2016.10.14 2020/01/29 荷積み明細対応
        Case ACT_KENPIN_Drct
            Select Case ID_KANRI_TBL(ING_No).Sagyo_Code.CODE_TYPE & ID_KANRI_TBL(ING_No).Sagyo_Code.YOIN_CODE
            Case "Q4"
                If nidumi(Sendbuf, i, j) Then
                    Exit Function
                End If
            Case Else
