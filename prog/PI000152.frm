VERSION 5.00
Begin VB.Form PI000152 
   Appearance      =   0  'ﾌﾗｯﾄ
   Caption         =   "商品化指図書発行＜構成部品登録＞([PI00015] 2016.06.22 08:45)"
   ClientHeight    =   10305
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   17010
...
Private Const pcmbD_JGYOBU01% = 0           '①　事業部　2016.01.27
Private Const pcmbD_JGYOBU02% = 1           '②　事業部　2016.01.27
Private Const pcmbD_JGYOBU03% = 2           '③　事業部　2016.01.27
Private Const pcmbD_JGYOBU04% = 3           '④　事業部　2016.01.27
Private Const pcmbD_JGYOBU05% = 4           '⑤　事業部　2016.01.27
Private Const pcmbD_JGYOBU06% = 5           '⑥　事業部　2016.01.27
Private Const pcmbD_JGYOBU07% = 6           '⑦　事業部　2016.01.27
Private Const pcmbD_JGYOBU08% = 7           '⑧　事業部　2016.01.27
Private Const pcmbD_JGYOBU09% = 8           '⑨　事業部　2016.01.27
Private Const pcmbD_JGYOBU10% = 9           '⑩　事業部　2016.01.27
Private Const pcmbD_JGYOBU11% = 10          '⑪　事業部　2016.01.27
Private Const pcmbD_JGYOBU12% = 11          '⑫　事業部　2016.01.27
Private Const pcmbD_JGYOBU13% = 12          '⑬　事業部　2016.01.27
Private Const pcmbD_JGYOBU14% = 13          '⑭　事業部　2016.01.27
Private Const pcmbD_JGYOBU15% = 14          '⑮　事業部　2016.01.27
Private Const pcmbD_JGYOBU16% = 15          '⑯　事業部　2016.01.27
Private Const pcmbD_JGYOBU17% = 16          '⑰　事業部　2016.01.27
Private Const pcmbD_JGYOBU18% = 17          '⑱　事業部　2016.01.27
Private Const pcmbD_JGYOBU19% = 18          '⑲　事業部　2016.01.27
Private Const pcmbD_JGYOBU20% = 19          '⑳　事業部　2016.01.27
Private Const pcmbD_JGYOBU21% = 20          '21　事業部　2016.01.27
Private Const pcmbD_JGYOBU22% = 21          '22　事業部　2016.01.27
Private Const pcmbD_JGYOBU23% = 22          '23　事業部　2016.01.27
Private Const pcmbD_JGYOBU24% = 23          '24　事業部　2016.01.27
Private Const pcmbD_JGYOBU25% = 24          '25　事業部　2016.01.27
...
Dim j   As Integer          '2016.01.27
    ...
    For i = pcmbD_SYUBETSU01 To pcmbD_SYUBETSU25
        'If Code_Set_Proc(i, P_KBN06_CD, 1) Then        '2017.09.27
        If Code_Set_Proc(i, P_KBN06_CD, 0) Then         '2017.09.27
            Unload Me
        End If
    Next i
...
'--------------------------------------------------- 事業部　セット 2016.01.27
    For i = pcmbD_JGYOBU01 To pcmbD_JGYOBU25
...
Dim wkJgyobu    As String * 1
    ...
    Do
        ...
        Next i
                    ...
                    '事業部 2016.01.27
        Combo2(Gyo).ListIndex = -1
        For i = 0 To Combo2(Gyo).ListCount - 1

        
            If D_Item_Tbl(SEQNO - 1).JGYOBU = Right(Combo2(Gyo).List(i), 1) Then
                Combo2(Gyo).ListIndex = i
                Exit For
            End If
        ...
        If Text1(text).text <> "" Then
            '品名
'>>>>>>>>>>>>>>>>>>>>>>>>>> 2013.01.07
            'If D_Item_Tbl(SEQNO - 1).JGYOBU = SHIZAI Then
...
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 2016.01.27
'            Select Case D_Item_Tbl(SEQNO - 1).JGYOBU
'                Case SHIZAI
'                    Call UniCode_Conv(K0_ITEM.JGYOBU, BUZAI)
'                Case SETSUBI
...
Dim S_NO        As Integer      '2016.01.27
...
Dim wkJgyobu    As String * 1
    ...
    Select Case Mode
        ...
        Case ptxD_HIN_GAI01, ptxD_HIN_GAI02, ptxD_HIN_GAI03, ptxD_HIN_GAI04, ptxD_HIN_GAI05, ptxD_HIN_GAI06, ptxD_HIN_GAI07, ptxD_HIN_GAI08, ptxD_HIN_GAI09, ptxD_HIN_GAI10, _
            ...
            Else
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  2014.03.24
'                Call UniCode_Conv(K0_ITEM.JGYOBU, Mid(Right(PI000151.Combo1(pcmbSHIMUKE).text, 4), 3, 1))
'                Call UniCode_Conv(K0_ITEM.NAIGAI, Mid(Right(PI000151.Combo1(pcmbSHIMUKE).text, 4), 4, 1))
...
'
'                        '品番（内）で読み替え
'                        Call UniCode_Conv(K2_ITEM.JGYOBU, Mid(Right(PI000151.Combo1(pcmbSHIMUKE).text, 4), 3, 1))
'                        Call UniCode_Conv(K2_ITEM.NAIGAI, Mid(Right(PI000151.Combo1(pcmbSHIMUKE).text, 4), 4, 1))
...
'                sts = Item_Read_Proc(Mid(Right(PI000151.Combo1(pcmbSHIMUKE).text, 4), 3, 1), Mid(Right(PI000151.Combo1(pcmbSHIMUKE).text, 4), 4, 1), Text1(Mode).text)
                ...
                Call UniCode_Conv(K0_ITEM.JGYOBU, Right(Combo2(S_NO).text, 1))
                Call UniCode_Conv(K0_ITEM.NAIGAI, Mid(Right(PI000151.Combo1(pcmbSHIMUKE).text, 4), 4, 1))
                Call UniCode_Conv(K0_ITEM.HIN_GAI, Text1(Mode).text)
                sts = BTRV(BtOpGetEqual, ITEM_POS, ITEMREC, Len(ITEMREC), K0_ITEM, Len(K0_ITEM), 0)
'>>>>>>>>>>>>>>>>>>>>>>>>>  品番読込み変更 2016.01.27
...
'>>>>>>>>>>>>>>>>>>>>>>>>>  2013.01.07
                ...
                '在庫数
'2016.01.27                If StrConv(ITEMREC.JGYOBU, vbUnicode) = SHIZAI Then
'2016.01.27                    wkJgyobu = BUZAI
'2016.01.27                Else
'2016.01.27                    wkJgyobu = StrConv(ITEMREC.JGYOBU, vbUnicode)      '2012.04.04      --> INS 2013.01.07
                    'wkJgyobu = YUKO_JGYOBU                              '2012.04.04    --> DEL 2013.01.07
'2016.01.27                End If
        
        
                wkJgyobu = StrConv(ITEMREC.JGYOBU, vbUnicode)      '2016.01.27
            ...
            Else
                ...
                Else
                    ...
                    Else
                        ...
                        '数量
                        If IsNumeric(PI000151.Text1(ptxSHIJI_QTY).text) Then
                            Text1(Mode + 1).text = Format(CDbl(CLng(PI000151.Text1(ptxSHIJI_QTY).text) * CDbl(Text1(Mode).text)), "#0.00")
