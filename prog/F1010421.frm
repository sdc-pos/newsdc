VERSION 5.00
Begin VB.Form F1010421 
   BackColor       =   &H00FFFFFF&
   Caption         =   "初期設定在庫登録（袋井用）"
   ClientHeight    =   6312
   ClientLeft      =   1920
   ClientTop       =   2436
   ClientWidth     =   11220
...
'　　　　単位のＩ－Ｏになる。
'#################################################################################################
Private Type YUKO_SOKO_TBL             '有効ﾎｽﾄ倉庫取り込みテーブル（洗濯機事業部）
    HS_SOKO As String * 2
    NAIGAI As String * 1
End Type
Dim SOKO_T1(0 To 9) As YUKO_SOKO_TBL
...
Dim NG_QTY1 As Long                     '無効データ件数（洗濯機）
                                    '画面初期表示（処理済「便」表示など）
...
Dim Cp_To As String
    ...
    If Data_Load = False Then       '便別取込みワークにﾃﾞｰﾀﾛｰﾄﾞ
        ...
        End If
                            '印刷フォント設定
        Printer.Font.NAME = F1010421.Font.NAME
        Printer.Font.Size = F1010421.Font.Size
...
'画面項目ロック解除　＆　終了メッセージ表示
    ...
    CntLab(ZERO).Caption = Format(NG_QTY1, "0") & " 件"
...
'洗濯機事業部  在庫設定データ取込み
    If HS_ZAI_Open1(ZERO, Work) = False Then   'ﾌｧｲﾙ無しなら処理しない
        Close #HS_ZAI_No
        ...
        Call Data_Load_Sub                  '洗濯機事業部  在庫設定データ→取込みﾜｰｸ
...
Dim Work As String
    ...
    Do
        If HS_ZAI_Get1() Then          'ﾎｽﾄﾃﾞｰﾀ 読込み
            Exit Do
        End If
...
'ホスト倉庫　範囲外？
        For i = ZERO To UBound(SOKO_T1) - 1
            If SOKO_T1(i).HS_SOKO = "  " Then
                ...
                Exit For
            End If
            If RTrim(StrConv(WK_ZAIREC.HOST_SOKO, vbUnicode)) = RTrim(SOKO_T1(i).HS_SOKO) Then
                Exit For
            End If
        Next i
...
Dim com     As String
    ...
    i = ZERO
    Do
        If GetIni("INIZAI_OK_SOKO", "SOKO1" & RTrim(Format(i + 1, "#0")), "SYS", c) Then
            Beep
        ...
        If GetIni("INIZAI_OK_SOKO", "NAIG1" & RTrim(Format(i + 1, "#0")), "SYS", c) Then
            Beep
            MsgBox "システム異常が発生しました。処理を中止して下さい。"
...
Dim i As Integer
    ...
    F1010421.MousePointer = vbHourglass

    Call Ctrl_Lock(F1010421)
...
Dim i As Integer
    ...
    Call Ctrl_UnLock(F1010421)


    F1010421.MousePointer = vbDefault
