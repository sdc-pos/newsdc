...
Private LABEL_GO_F      As Boolean          'ラベル発行画面への推移 2013.03.28
...
Dim TORI_GENSANKOKU     As String   '2013.03.27
...
'            chk_TORI_GENSANKOKU = StrConv(ITEMREC.TORI_GENSANKOKU, vbUnicode)           '原産国有無ﾁｪｯｸ用   2013.01.08
            ...
            TORI_GENSANKOKU = ""
            For i = 1 To Len(StrConv(ITEMREC.TORI_GENSANKOKU, vbUnicode))
                        
                If Mid(StrConv(ITEMREC.TORI_GENSANKOKU, vbUnicode), i, 1) < " " Then
                    TORI_GENSANKOKU = TORI_GENSANKOKU & " "
                Else
                    TORI_GENSANKOKU = TORI_GENSANKOKU & Mid(StrConv(ITEMREC.TORI_GENSANKOKU, vbUnicode), i, 1)
                End If
...
'2013.03.27            If Trim(StrConv(ITEMREC.TORI_GENSANKOKU, vbUnicode)) = "" Then
            If Trim(TORI_GENSANKOKU) = "" Then
            Else
                ...
                'Call UniCode_Conv(K0_GENSAN.GENSANKOKU, StrConv(ITEMREC.TORI_GENSANKOKU, vbUnicode))   '2013.03.31
                ...
                Select Case sts
                    ...
                    Case BtErrKeyNotFound
                        ...
                        'Call UniCode_Conv(GENSANREC.GENSANKOKU, StrConv(ITEMREC.TORI_GENSANKOKU, vbUnicode))   '2013.03.31
            ...
            Do
                ...
                If StrConv(GENSANREC.UPD_DATETIME, vbUnicode) = "" Then
                    lstGensankoku.AddItem StrConv(GENSANREC.Ins_DateTime, vbUnicode) & StrConv(GENSANREC.GENSANKOKU, vbUnicode)     '2013.02.19
                Else                                                                                                                '2013.02.19
                    lstGensankoku.AddItem StrConv(GENSANREC.UPD_DATETIME, vbUnicode) & StrConv(GENSANREC.GENSANKOKU, vbUnicode)     '2013.02.19
                End If
            ...
            Else
                ...
                Else
                    ...
                    If Trim(Text1(ptxLabel_QTY).text) = "" Then '2008.02.06
                        Text1(ptxLabel_QTY).text = Format(CLng(Text1(ptxSHIJI_QTY).text) + 1, "#0")
                    End If
...
'2010.11.12 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    Do
        ...
        If Trim(StrConv(GENSANREC.UPD_DATETIME, vbUnicode)) = "" Then                                                       '2013.01.28
            lstGensankoku.AddItem StrConv(GENSANREC.Ins_DateTime, vbUnicode) & StrConv(GENSANREC.GENSANKOKU, vbUnicode)     '2013.01.28
        Else                                                                                                                '2013.01.28
            lstGensankoku.AddItem StrConv(GENSANREC.UPD_DATETIME, vbUnicode) & StrConv(GENSANREC.GENSANKOKU, vbUnicode)     '2013.01.28
        End If
...
Dim wkMSG       As String       '2013.04.03
...
''''2011.02.17
                            ...
                            If KAISYA_CHK_F Then
                                ...
                                'If Trim(lblL_KAISHA.Caption) = "" Or Trim(lblL_JGYOBU.Caption) = "" Then       '2013.04.03
                                                                                                                '2013.04.03
                                If Trim(lblL_KAISHA.Caption) = "" Or Trim(lblL_JGYOBU.Caption) = "" Or Trim(txGensankoku.text) = "" Then
                                    wkMSG = ""
                                    If Trim(lblL_KAISHA.Caption) = "" Then
                                        wkMSG = "[会社名]"
                                    End If
                                    If Trim(lblL_JGYOBU.Caption) = "" Then
                                        wkMSG = "[事業部]"
                                    End If
                                    ...
                                    ans = MsgBox("会社名/事業部/原産国 が指定されていません。" & vbCrLf & _
                                            wkMSG & vbCrLf & _
                                        "(ＯＫ＝発行、ｷｬﾝｾﾙ=発行しない)", vbOKCancel + vbQuestion + vbDefaultButton2, "確認入力")
                                    If ans = vbCancel Then
' 2013.01.05 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
'                                        L_print_Flg = False
                                        Text1(ptxHIN_GAI).SetFocus
                                    Else
                                        'sts = Shell(App.Path & "\PM00040.exe " & Right(Combo1(pcmbSHIMUKE).text, 2) & Trim(Text1(ptxHIN_GAI).text), vbNormalFocus)
                                        
                                        
                                        If LABEL_GO_F Then      '2013.04.03
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   DEBUG
                                            sts = Shell(App.Path & "\PM00040.exe " & Right(Combo1(pcmbSHIMUKE).text, 2) & Trim(Text1(ptxHIN_GAI).text), vbNormalFocus)
                                        End If                  '2013.04.03
                                    End If
...
' 2013.01.05 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                            ...
                            If KISHU_CHECK Then
                                ...
                                If Trim(KISHU1) = "" And Trim(KISHU2) = "" Then
                                    ans = MsgBox("代表機種が空白です。(ＯＫ＝印刷中止、ｷｬﾝｾﾙ=継続)", vbOKCancel + vbQuestion, "確認入力")
                                    If ans = vbCancel Then
                                    Else
                                        L_print_Flg = False
                                    End If
...
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 自動設定ｺｰﾄﾞ　会社／事業部 2013.03.28
    If GetIni(App.EXEName, "KAISYA_DEF_VALUE", App.EXEName, c) Then
        KAISYA_DEF_VALUE = ""
    Else
        KAISYA_DEF_VALUE = Trim(c)
    End If

    If GetIni(App.EXEName, "JIGYOBU_DEF_VALUE", App.EXEName, c) Then
        JIGYOBU_DEF_VALUE = ""
    Else
...
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ラベル発行画面への推移 2013.03.28
    If GetIni(App.EXEName, "LABEL_GO_F", App.EXEName, c) Then
        LABEL_GO_F = False
    Else
...
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ラベル発行画面への推移 2013.03.28
    ...
    PI000101.Caption = PI000101.Caption & Last_Update_Day
