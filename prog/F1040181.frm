...
Begin VB.Form F1040181 
   Caption         =   "[緊急処理]在庫データ設定単価入れ替え処理"
   ClientHeight    =   6675
   ClientLeft      =   2025
   ClientTop       =   -3510
   ClientWidth     =   8985
...
Dim MESG    As String
    ...
    Select Case Index
        ...
        Case 1
            ...
            ans = MsgBox("単価入れ替え処理を実行しますか？", vbYesNo, "確認入力")
...
Dim sts     As Integer
    ...
    F1040181.Caption = F1040181.Caption & " " & LAST_UPDATE_DAY
...
Dim i   As Integer
    ...
    F1040181.MousePointer = vbHourglass

    Call Ctrl_Lock(F1040181)
...
Dim i   As Integer
    ...
    Call Ctrl_UnLock(F1040181)


    F1040181.MousePointer = vbDefault
...
Dim cnt         As Long
    ...
    Do While Not EOF(FileNo)
        ...
        If UBound(In_wk) < 10 Then
        Else
        
            If In_wk(10) <> "*" Then
            Else
        
                If IsNumeric(In_wk(9)) Then
        
        
        
                    If Len(In_wk(0)) < 7 Then
            
                        In_wk(0) = "0" & In_wk(0)
                    End If
                    
                    Call UniCode_Conv(K0_ZAIKO.Soko_No, Mid(Trim(CStr(In_wk(0))), 1, 2))
                    Call UniCode_Conv(K0_ZAIKO.Retu, Mid(Trim(CStr(In_wk(0))), 3, 2))
                    Call UniCode_Conv(K0_ZAIKO.Ren, Mid(Trim(CStr(In_wk(0))), 5, 2))
                    Call UniCode_Conv(K0_ZAIKO.Dan, Mid(Trim(CStr(In_wk(0))), 7, 2))
            
                    Call UniCode_Conv(K0_ZAIKO.JGYOBU, Trim(CStr(In_wk(1))))
                    
                    Call UniCode_Conv(K0_ZAIKO.NAIGAI, Trim(CStr(In_wk(2))))
            
                    Call UniCode_Conv(K0_ZAIKO.HIN_GAI, Trim(CStr(In_wk(3))))
            
                    Call UniCode_Conv(K0_ZAIKO.GOODS_ON, Trim(CStr(In_wk(4))))
            
                    Call UniCode_Conv(K0_ZAIKO.NYUKA_DT, Trim(CStr(In_wk(5))))
                    ...
                    Select Case sts
                        Case BtNoErr
                            ...
                            List_Wk = CStr(In_wk(0)) & " " & CStr(In_wk(1)) & " " & CStr(In_wk(2)) & " " & CStr(In_wk(3)) & " " & CStr(In_wk(4)) & " " & CStr(In_wk(5)) & Trim(StrConv(ZAIKOREC.SHIIRE_CODE, vbUnicode)) & " " & Trim(StrConv(ZAIKOREC.SHIIRE_TANKA, vbUnicode))
                                            
                            
'                            Call UniCode_Conv(ZAIKOREC.SHIIRE_CODE, Trim(CStr(In_wk(7))))
                        
                            Call UniCode_Conv(ZAIKOREC.SHIIRE_TANKA, Format(CCur(Trim(CStr(In_wk(9)))), "00000000.00"))
                        
                            List_Wk = List_Wk & " -> " & Trim(StrConv(ZAIKOREC.SHIIRE_CODE, vbUnicode)) & " " & Trim(StrConv(ZAIKOREC.SHIIRE_TANKA, vbUnicode))
