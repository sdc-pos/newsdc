...
Begin VB.Form F1050302 
   ...
   Begin TrueDBGrid80.TDBGrid TDBGrid1 
      Height          =   1815
      Left            =   1920
      OleObjectBlob   =   "F1050302.frx":0000
      TabIndex        =   12
      Top             =   1440
      Width           =   4215
   End
...
Dim Soko_No     As String * 2
    ...
    Open Trim(GW_Path) For Output As FileNo
                                'ヘッダ出力
    Write #FileNo, "事業部", "国内外", "品番(外部)", "品名", "要因", "特", "実績日付", "実績時刻", "伝票日付", "伝票№", "総在庫数", "入庫数(済)", "入庫数(未)", "入庫数(済)", "入庫数(未)", "在訂(済)", "在訂(未)", "移動(済)", "移動(未)", "入出庫元", "入出庫先", "入荷日", "向け先", "ID", "品番(内部)", "メモ", "伝票ＩＤ", "担当者CD", "担当者名", "標準棚番"
