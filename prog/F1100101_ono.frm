...
Private Type ID_KANRI_TBL_tag               '子機情報の管理
    ...
    MENU_LV1                    As String * 3           'メニューレベル１
    MENU_LV2                    As String * 3           'メニューレベル２
    MENU_LV3                    As String * 3           'メニューレベル３
    
    PageNo_LV1                  As Integer              'ページ№（メニュー）
    PageNo_LV2                  As Integer              'ページ№（メニュー）
    PageNo_LV3                  As Integer              'ページ№（メニュー）
...
Private Const Step_MENU3_REQ% = 14      'メニュー３要求
Private Const Step_MENU3_RES% = 15      'メニュー３回答
...
Private Type Menu_Tbl_tag               'メニュー送信用テーブル
    CODE        As String * 3
    Disp        As String * 20          '2006.06.01 10-->20
End Type
...
End Type
                                        '※作業宣言の数により増減必須！！
Private WEL_Para_Tbl(0 To 14, 0 To 8) As WEL_Para_Tbl_tag
...
'                        ID_KANRI_TBL(ING_No).Step = Step_Start
    ...
    '                                Unload Me
                        ...
                        Case Step_MENU1_REQ, Step_MENU2_REQ, Step_MENU3_REQ
                             ...
                             Select Case Trim(ID_KANRI_TBL(ING_No).Recv_text(0))
                                Case BEF_Page       '前頁
                                    ...
                                    Select Case ID_KANRI_TBL(ING_No).Step
                                        ...
                                        Case Step_MENU3_REQ
                                            ID_KANRI_TBL(ING_No).PageNo_LV3 = ID_KANRI_TBL(ING_No).PageNo_LV3 - 1
                                    End Select
...
'                                            ID_KANRI_TBL(ING_No).PageNo_LV3 = 0
                                    ...
                                    Select Case ID_KANRI_TBL(ING_No).Step
                                        ...
                                        Case Step_MENU3_RES
                                            ID_KANRI_TBL(ING_No).MENU_LV3 = Trim(ID_KANRI_TBL(ING_No).Recv_text(0))
                                    End Select
...
'-------------------------------------------------------
    ...
    Select Case ID_KANRI_TBL(ING_No).Step
        ...
        Case Step_MENU1_REQ, Step_MENU2_REQ, Step_MENU3_REQ
                                        'メニュー回答
        Case Step_MENU1_RES, Step_MENU2_RES, Step_MENU3_RES
...
'----------------------------------------------- '専用メニュー獲得
    If Menu_Type = 1 Then
                        '共通メニュー
    Else
        Call UniCode_Conv(K0_TMENU.TANTO_CODE, ID_KANRI_TBL(ING_No).TANTO_CODE)
        sts = BTRV(BtOpGetEqual, TMENU_POS, TMENUREC, Len(TMENUREC), K0_TMENU, Len(K0_TMENU), 0)
        Select Case sts
            Case BtNoErr
                ID_KANRI_TBL(ING_No).MENU_GRP = StrConv(TMENUREC.MENU_GRP_NO, vbUnicode)
            Case BtErrKeyNotFound
...
Dim WK_LV3      As String * 3


    Menu_Send_Proc = True
'----------------------------------------------- '事業部選択あり
    If ID_KANRI_TBL(ING_No).JGYOBU = " " Then
...
'----------------------------------------------- '国内外選択あり
    ...
    End If
'----------------------------------------------- 'メニュー管理の読み込み
    If Len(Trim(ID_KANRI_TBL(ING_No).MENU_GRP)) = 0 Then
                                    'ここで未確定なのは共通メニュー時だけ
        Call UniCode_Conv(K1_MENU.JGYOBU, ID_KANRI_TBL(ING_No).JGYOBU)
        Call UniCode_Conv(K1_MENU.NAIGAI, ID_KANRI_TBL(ING_No).NAIGAI)
        Call UniCode_Conv(K1_MENU.MENU_GRP_NO, ALL_MENU_GRP)
        
        Call UniCode_Conv(K1_MENU.MENU_LV1, "")
        Call UniCode_Conv(K1_MENU.MENU_LV2, "")
        Call UniCode_Conv(K1_MENU.MENU_LV3, "")
        ...
        sts = BTRV(BtOpGetGreaterEqual, MENU_POS, MENUREC, Len(MENUREC), K1_MENU, Len(K1_MENU), 1)
        Select Case sts
            Case BtNoErr
            Case BtErrEOF
            ...
            Case Else
                Call Err_Send_Proc("システム異常発生", "", "", "", "")
                Sendbuf = Text_Create_Proc()
                Call File_Error(sts, BtOpGetEqual, "メニュー管理マスタ", 0)
                Exit Function
        End Select
    
        ID_KANRI_TBL(ING_No).MENU_GRP = StrConv(MENUREC.MENU_GRP_NO, vbUnicode)
    ...
    End If
    '   -------------------------------- メニュー管理マスタ読込み
    Call UniCode_Conv(K0_MENU.MENU_GRP_NO, ID_KANRI_TBL(ING_No).MENU_GRP)
    Call UniCode_Conv(K0_MENU.JGYOBU, ID_KANRI_TBL(ING_No).JGYOBU)
    Call UniCode_Conv(K0_MENU.NAIGAI, ID_KANRI_TBL(ING_No).NAIGAI)
    Call UniCode_Conv(K0_MENU.MENU_LV1, ID_KANRI_TBL(ING_No).MENU_LV1)
    Call UniCode_Conv(K0_MENU.MENU_LV2, ID_KANRI_TBL(ING_No).MENU_LV2)
    Call UniCode_Conv(K0_MENU.MENU_LV3, ID_KANRI_TBL(ING_No).MENU_LV3)
    ...
    Do
        ...
        Select Case sts
            Case BtNoErr
                If ID_KANRI_TBL(ING_No).MENU_GRP <> StrConv(MENUREC.MENU_GRP_NO, vbUnicode) Or _
                    ID_KANRI_TBL(ING_No).JGYOBU <> StrConv(MENUREC.JGYOBU, vbUnicode) Or _
                    ID_KANRI_TBL(ING_No).NAIGAI <> StrConv(MENUREC.NAIGAI, vbUnicode) Then
                    Exit Do
                End If
            
                WK_LV1 = ID_KANRI_TBL(ING_No).MENU_LV1
                WK_LV2 = ID_KANRI_TBL(ING_No).MENU_LV2
                WK_LV3 = ID_KANRI_TBL(ING_No).MENU_LV3
                
                
                If Len(Trim(ID_KANRI_TBL(ING_No).MENU_LV1)) = 0 Then
                    WK_LV1 = StrConv(MENUREC.MENU_LV1, vbUnicode)
                    WK_LV2 = ""
                    WK_LV3 = ""
...
'                    ID_KANRI_TBL(ING_No).PageNo_LV1 = 0
                ...
                Else
                    If Len(Trim(ID_KANRI_TBL(ING_No).MENU_LV2)) = 0 Then
                        WK_LV2 = StrConv(MENUREC.MENU_LV2, vbUnicode)
                        WK_LV3 = ""
...
'                        ID_KANRI_TBL(ING_No).PageNo_LV2 = 0
                    ...
                    Else
                        If Len(Trim(ID_KANRI_TBL(ING_No).MENU_LV3)) = 0 Then
                            WK_LV3 = StrConv(MENUREC.MENU_LV3, vbUnicode)
...
'                            ID_KANRI_TBL(ING_No).PageNo_LV3 = 0
    ...
    '   -------------------------------- メニュー情報を保存
        If WK_LV1 <> StrConv(MENUREC.MENU_LV1, vbUnicode) Or _
            WK_LV2 <> StrConv(MENUREC.MENU_LV2, vbUnicode) Or _
            WK_LV3 <> StrConv(MENUREC.MENU_LV3, vbUnicode) Then
        Else
            ...
            If Len(Trim(ID_KANRI_TBL(ING_No).MENU_LV1)) = 0 Then
                Menu_Tbl(Menu_Cnt).CODE = StrConv(MENUREC.MENU_LV1, vbUnicode)
            Else
                If Len(Trim(ID_KANRI_TBL(ING_No).MENU_LV2)) = 0 Then
                    Menu_Tbl(Menu_Cnt).CODE = StrConv(MENUREC.MENU_LV2, vbUnicode)
                Else
                    If Len(Trim(ID_KANRI_TBL(ING_No).MENU_LV3)) = 0 Then
                        Menu_Tbl(Menu_Cnt).CODE = StrConv(MENUREC.MENU_LV3, vbUnicode)
                    End If
                End If
            End If
            
        
            Menu_Tbl(Menu_Cnt).Disp = StrConv(MENUREC.DISPLAY_ITEM, vbUnicode)
        End If
...
'----------------------------------------------- 'メニュー送信テキスト作成
'''    ID_KANRI_TBL(ING_No).Step = ID_KANRI_TBL(ING_No).Step + 1   'メニュー送信
    '---------------------------------------------------------------
...
'    Start_Gyo = ID_KANRI_TBL(ING_No).PageNo * M_Gyo
'    End_Gyo = (ID_KANRI_TBL(ING_No).PageNo * M_Gyo) + (M_Gyo - 1)


    Select Case ID_KANRI_TBL(ING_No).Step
        Case Step_MENU1_REQ, Step_MENU1_RES
            PageNo = ID_KANRI_TBL(ING_No).PageNo_LV1
        Case Step_MENU2_REQ, Step_MENU2_RES
            PageNo = ID_KANRI_TBL(ING_No).PageNo_LV2
        Case Step_MENU3_REQ, Step_MENU3_RES
            PageNo = ID_KANRI_TBL(ING_No).PageNo_LV3
    End Select
    ...
    For i = Start_Gyo To End_Gyo
        ...
        Else
            ...
            Send_Text.Box_Type(j).MENU = Menu_Tbl(i).CODE       'メニュ―番号
            ID_KANRI_TBL(ING_No).Send_Text.Box_Type(j).MENU = Menu_Tbl(i).CODE
...
Dim sts As Integer
    ...
    Call UniCode_Conv(K1_MENU.JGYOBU, ID_KANRI_TBL(ING_No).JGYOBU)
    Call UniCode_Conv(K1_MENU.NAIGAI, ID_KANRI_TBL(ING_No).NAIGAI)
    Call UniCode_Conv(K1_MENU.MENU_GRP_NO, ID_KANRI_TBL(ING_No).MENU_GRP)
    
    If ID_KANRI_TBL(ING_No).Step = Step_MENU1_REQ Then
        Call UniCode_Conv(K1_MENU.MENU_LV1, "000")
    Else
        Call UniCode_Conv(K1_MENU.MENU_LV1, ID_KANRI_TBL(ING_No).MENU_LV1)
    End If
    
    Call UniCode_Conv(K1_MENU.MENU_LV2, ID_KANRI_TBL(ING_No).MENU_LV2)
    Call UniCode_Conv(K1_MENU.MENU_LV3, ID_KANRI_TBL(ING_No).MENU_LV3)
    sts = BTRV(BtOpGetEqual, MENU_POS, MENUREC, Len(MENUREC), K1_MENU, Len(K1_MENU), 1)
    Select Case sts
        Case BtNoErr
        Case BtErrKeyNotFound
    ...
    If Trim(ID_KANRI_TBL(ING_No).Recv_text(0)) <> BEF_Page And Trim(ID_KANRI_TBL(ING_No).Recv_text(0)) <> NEXT_Page And _
     StrConv(MENUREC.MENU_KBN, vbUnicode) = "1" Then
                                                                                                                                    
                                            
                                            '要因の読込み
        Call UniCode_Conv(K0_YOIN.CODE_TYPE, StrConv(MENUREC.CODE_TYPE, vbUnicode))
        Call UniCode_Conv(K0_YOIN.YOIN_CODE, StrConv(MENUREC.YOIN_CODE, vbUnicode))
        sts = BTRV(BtOpGetEqual, YOIN_POS, YOINREC, Len(YOINREC), K0_YOIN, Len(K0_YOIN), 0)
        Select Case sts
            Case BtNoErr
                ID_KANRI_TBL(ING_No).YOIN_DNAME = StrConv(YOINREC.YOIN_DNAME, vbUnicode)
            
                If StrConv(YOINREC.PARAM_F, vbUnicode) = "1" Then   '向け先なら（出荷）
                    ID_KANRI_TBL(ING_No).CYU_KBN = StrConv(MENUREC.YOIN_CODE, vbUnicode)
                    ID_KANRI_TBL(ING_No).MTS_CODE = Left(StrConv(MENUREC.PARAM, vbUnicode), 8)
                    ID_KANRI_TBL(ING_No).SS_CODE = Right(StrConv(MENUREC.PARAM, vbUnicode), 8)
                End If
                                                                    '検品（向け先指定）なら
                ...
                ID_KANRI_TBL(ING_No).Sagyo_Code.PARAM = StrConv(MENUREC.PARAM, vbUnicode)
...
'                sts = BTRV(BtOpGetEqual, ITEM_POS, ITEMREC, Len(ITEMREC), K0_ITEM, Len(K0_ITEM), 0)
    ...
    End If
                                        '入庫更新
'2007.05.28    sts = Nyuko_Update_Proc(RET_JGYOBU, _
'2007.05.28                            RET_NAIGAI, _
'2007.05.28                            Hinban, _
'2007.05.28                            Format(Now, "YYYYMMDD"), _
'2007.05.28                            Tanaban, _
'2007.05.28                            (ID_KANRI_TBL(ING_No).Sagyo_Code.CODE_TYPE & ID_KANRI_TBL(ING_No).Sagyo_Code.YOIN_CODE), _
'2007.05.28                            SUMI_QTY, _
'2007.05.28                            MI_QTY, _
'2007.05.28                            Format(ID_KANRI_TBL(ING_No).ID, "000"), _
'2007.05.28                            ID_KANRI_TBL(ING_No).TANTO_CODE, _
'2007.05.28                            FILE_RETRY)
'2007.05.28    Select Case sts
'2007.05.28        Case False
'2007.05.28        Case True           '入庫時は発生しない
'2007.05.28        Case SYS_CANCEL
'2007.05.28            Call Err_Send_Proc(ID_KANRI_TBL(ING_No).YOIN_DNAME, "処理中断", "", "", "")
'2007.05.28            Sendbuf = Text_Create_Proc()
'2007.05.28            ID_KANRI_TBL(ING_No).Step = ID_KANRI_TBL(ING_No).Step - 1
'2007.05.28
'2007.05.28            Zaitei_In_Proc = False
'2007.05.28            GoTo Abort_Tran
'2007.05.28        Case SYS_ERR
'2007.05.28            Sendbuf = Text_Create_Proc()
'2007.05.28            Call Err_Send_Proc("システム異常発生", "", "", "", "")
'2007.05.28            Zaitei_In_Proc = SYS_ERR    'システム異常発生
'2007.05.28
'2007.05.28            GoTo Abort_Tran
'2007.05.28    End Select
...
'''                            End If
                        ...
                        Call UniCode_Conv(ID_KANRI_TBL(ING_No).Send_Text.Box_Type(1).LCD, "伝票ＩＤ:" & ID_KANRI_TBL(ING_No).ID_NO)
                                                                                '数値初期表示
                        Send_Text.Box_Type(1).INIT = ""
                        ID_KANRI_TBL(ING_No).Send_Text.Box_Type(1).INIT = ""
                                                                                '初期カーソル位置
            ...
            For i = 0 To M_Gyo - 1
                ...
                Select Case Trim(WEL_Para_Tbl(Ti, Tj).Wel_Para(i).LCD)
                    Case LCD_ID_No      '伝票ＩＤ
                        ...
                        If Len(Trim(ID_KANRI_TBL(ING_No).Recv_text(i))) = 13 Then
                            ID_NO = Trim(ID_KANRI_TBL(ING_No).Recv_text(i))
                            Call Err_Send_Proc(ID_KANRI_TBL(ING_No).YOIN_DNAME, ID_NO, "検品作業不可", "", "")
                            Sendbuf = Text_Create_Proc()
                            ID_KANRI_TBL(ING_No).Step = ID_KANRI_TBL(ING_No).Step - 1
                            Inspe_Proc = False
                            Exit Function
...
Private Function Y_Syuka_Chek_Proc(Mode As String, _
                                        ...
                                        KAN_KBN As String, _
                                        Optional JITU_QTY, _
                                        Optional DEN_ID_JGYOBU = " ") As Integer
'-------------------------------------------------------
'
'   『単一出荷予定の使用予約』
'
...
'                Call Start_Proc(Sendbuf)
        ...
        Case Step_Sagyo1_REQ    '作業１要求
            If Len(Trim(ID_KANRI_TBL(ING_No).MENU_LV3)) <> 0 Then
                ID_KANRI_TBL(ING_No).Step = Step_MENU3_REQ
                ID_KANRI_TBL(ING_No).MENU_LV3 = ""
                Call Menu_Send_Proc(Sendbuf)
            Else
                If Len(Trim(ID_KANRI_TBL(ING_No).MENU_LV2)) <> 0 Then
