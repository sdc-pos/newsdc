VERSION 5.00
Begin VB.Form F1020111 
   BackColor       =   &H00FFFFFF&
   Caption         =   "入出荷予定データ取込み「滋賀用」"
   ClientHeight    =   6324
   ClientLeft      =   1908
   ClientTop       =   2388
   ClientWidth     =   11220
...
Private Type YUKO_SOKO_TBL7             '有効ﾎｽﾄ倉庫取り込みテーブル（掃除機事業部）
    ...
    NAIGAI As String * 1
End Type
Dim SOKO_T7(ZERO To 9) As YUKO_SOKO_TBL7
...
'重要　※　特売りは４便とみなす　※
    ...
    If Shori_Mode > ZERO Then
        ...
        If HS_SIJ_Open7(ZERO, Format(Shori_Mode, "0")) = False Then
                                                        'ﾌｧｲﾙ無しなら処理しない
            Close #HS_SIJ_No
                                                        '掃除機事業部ﾎｽﾄﾃﾞｰﾀ OPEN
            If HS_SIJ_Open7(1, Format(Shori_Mode, "0")) Then
                Unload Me
            End If
    ...
    Do
        ...
        For i = ZERO To UBound(SOKO_T7) - 1
            If SOKO_T7(i).HS_SOKO = "  " Then
                ...
                Exit For
            End If
            If RTrim(StrConv(HS_SIJREC.HOST_SOKO, vbUnicode)) = RTrim(SOKO_T7(i).HS_SOKO) Then
                Exit For
            End If
        Next i
...
'国内外区分の設定
    For i = ZERO To UBound(SOKO_T7)
        If SOKO_T7(i).HS_SOKO = "  " Then
            Exit For
        End If
        If RTrim(StrConv(XX_SIJREC.HOST_SOKO, vbUnicode)) = RTrim(SOKO_T7(i).HS_SOKO) Then
...
'強制再実行処理
    ...
    Text1(ZERO).Visible = True
                                                ...
                                                '掃除機 SPIC
    Call UniCode_Conv(K0_SEQCK.JGYOBU, "7")
    Call UniCode_Conv(K0_SEQCK.SEQ_MODE, "1")
...
Dim com     As String
    ...
    i = ZERO
    Do
        If GetIni("NYUSYU_OK_SOKO", "SOKO7" & RTrim(Format(i + 1, "#0")), "SYS", c) Then
            Beep
        ...
        If GetIni("NYUSYU_OK_SOKO", "NAIG7" & RTrim(Format(i + 1, "#0")), "SYS", c) Then
            Beep
            MsgBox "システム異常が発生しました。処理を中止して下さい。"
    ...
    End If
                                '印刷フォント設定
    With NormalFont
        .NAME = F1020111.FontName
        .Size = F1020111.FontSize
    End With
...
'----------------------------------------------------------------------------
    ...
    F1020111.MousePointer = vbHourglass

    Call Ctrl_Lock(F1020111)
...
'----------------------------------------------------------------------------
    ...
    Call Ctrl_UnLock(F1020111)

    F1020111.MousePointer = vbDefault
...
Dim ans As Integer
    ...
    If com = BtOpInsert Then
        Call UniCode_Conv(SEQCKREC.JGYOBU, "7")
        Call UniCode_Conv(SEQCKREC.SEQ_MODE, "1")
    End If
