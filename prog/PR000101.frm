...
Begin VB.Form PR000101 
   Caption         =   "売上実績検索・売上集計表発行"
   ClientHeight    =   10305
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   14985
...
'----------------------------------------------------------------------------
    ...
    PR000101.MousePointer = vbHourglass

    Call Ctrl_Lock(PR000101)
...
'----------------------------------------------------------------------------
    ...
    Call Ctrl_UnLock(PR000101)


    PR000101.MousePointer = vbDefault
...
Dim sts As Integer
    ...
    End If
    Set PR000101 = Nothing
    Set PR000102 = Nothing
    Set PR00010F1 = Nothing
    Set PR00010F2 = Nothing
...
Dim rpt             As New PR00010F1
Dim f               As New PR000102
    ...
    Do
        ...
        If Not Skip_Flg Then
            ...
            Select Case StrConv(P_SHURIAGE_REC.TORI_KBN, vbUnicode)
                Case P_TORI_SYANAI, P_TORI_ANOTHER
                Case Else
                    YMD = StrConv(P_SHURIAGE_REC.URIAGE_DT, vbUnicode)
                    
                    
                    
                    
                    If CLng(StrConv(P_SHURIAGE_REC.KINGAKU, vbUnicode)) >= 0 Then
                        If YMD < StrConv(P_KANRIREC.ZEI_CHANGE_YMD, vbUnicode) Then
                            ZEI = Int(CDbl(CLng(StrConv(P_SHURIAGE_REC.KINGAKU, vbUnicode)) * (CDbl(StrConv(P_KANRIREC.NOW_ZEI_RITU, vbUnicode)) / 100)) + _
                                    CDbl(CDbl(StrConv(P_KANRIREC.NOW_MARUME, vbUnicode)) / 10))
                        Else
                            ZEI = Int(CDbl(CLng(StrConv(P_SHURIAGE_REC.KINGAKU, vbUnicode)) * (CDbl(StrConv(P_KANRIREC.NEW_ZEI_RITU, vbUnicode)) / 100)) + _
                                    CDbl(CDbl(StrConv(P_KANRIREC.NEW_ZEI_RITU, vbUnicode)) / 10))
                        End If
                    Else
                        
                        wkKINGAKU = CLng(StrConv(P_SHURIAGE_REC.KINGAKU, vbUnicode)) * -1
                    ...
                    End If
                    '2007.05.02 緊急
'''                    If Trim(StrConv(P_SHURIAGE_REC.TOKUI_CODE, vbUnicode)) = "S230" Then
'''                        ZEI = 0
'''                    End If
            
                    TOTAL_URIKAKE(5) = TOTAL_URIKAKE(5) + ZEI
