VERSION 5.00
Begin {2AF752CD-B826-4828-B4C1-13BFC9CC90C1} PR00027F2 
   ClientHeight    =   9990
   ClientLeft      =   165
   ClientTop       =   555
   ClientWidth     =   15240
   ...
   _ExtentX        =   26882
   _ExtentY        =   17621
   SectionData     =   "PR00027F2.dsx":0000
End
Attribute VB_Name = "PR00027F2"
...
Dim sts         As Integer
    ...
    Select Case sts
        Case BtNoErr
            ...
            If StrConv(P_SHUKEIRE_REC.KEIJYO_YM, vbUnicode) <> Mid(PR000271.Text1(ptxKEIJYO_YM).Text, 1, 4) & _
                                                                Mid(PR000271.Text1(ptxKEIJYO_YM).Text, 6, 2) Then
                Exit Sub
            End If
        
            If Print_Mode = 1 Then
                If Trim(StrConv(P_SHUKEIRE_REC.ORDER_CODE, vbUnicode)) <> PR000271.Text1(ptxSHIIRE_CODE).Text Then
                    Exit Sub
                End If
            End If
    ...
    Select Case sts
        ...
        Case BtErrKeyNotFound
            ...
            Me.Fields("DET_HIN_NAME") = ""          '2016.02.26
...
Dim sts         As Integer
    ...
    If Trim(PR000271.Text1(ptxSHIIRE_CODE).Text) = "" Then
        Print_Mode = 0
    Else
        Print_Mode = 1
    End If
    ...
    If Print_Mode = 0 Then
        ...
        Call UniCode_Conv(K2_P_SHUKEIRE.KEIJYO_YM, Mid(PR000271.Text1(ptxKEIJYO_YM).Text, 1, 4) & _
                                                    Mid(PR000271.Text1(ptxKEIJYO_YM).Text, 6, 2))
        Call UniCode_Conv(K2_P_SHUKEIRE.UKEIRE_DT, "")
    
    Else

        Call UniCode_Conv(K1_P_SHUKEIRE.KEIJYO_YM, Mid(PR000271.Text1(ptxKEIJYO_YM).Text, 1, 4) & _
                                                    Mid(PR000271.Text1(ptxKEIJYO_YM).Text, 6, 2))
        Call UniCode_Conv(K1_P_SHUKEIRE.ORDER_CODE, PR000271.Text1(ptxSHIIRE_CODE).Text)
