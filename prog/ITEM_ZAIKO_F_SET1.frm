...
Begin VB.Form ITEM_ZAIKO_F_SET1 
   Caption         =   "品目ﾏｽﾀ在庫管理フラグ設定[ITEM_ZAIKO_F_SET]　2012.12.17 08:00"
   ClientHeight    =   9510
   ClientLeft      =   2025
   ClientTop       =   -5445
   ClientWidth     =   15210
   ...
   End
End
Attribute VB_Name = "ITEM_ZAIKO_F_SET1"
...
Dim wkTanaban       As String * 8
    ...
    For Row = 1 To PLN_Y_NYUKA.UpperBound(1)
        ...
        Call UniCode_Conv(K0_ITEM.JGYOBU, PLN_Y_NYUKA(Row, colJGYOBU))
        Call UniCode_Conv(K0_ITEM.NAIGAI, PLN_Y_NYUKA(Row, colNAIGAI))
        ...
        Select Case sts
            Case BtNoErr
                ...
                Call UniCode_Conv(ITEMREC.UPD_TANTO, App.EXEName)
                Call UniCode_Conv(ITEMREC.UPD_DATETIME, INS_NOW)
    ...
    For i = 1 To 1048576
        ...
        Else
            ...
            Else
                ...
                '品番
                wkHIN_NAME = Trim(xlSheet.Application.Cells(i, 3))
                '品番
...
Dim i   As Integer
    ...
    ITEM_ZAIKO_F_SET1.MousePointer = vbHourglass

    Call Ctrl_Lock(ITEM_ZAIKO_F_SET1)
...
Dim i   As Integer
    ...
    Call Ctrl_UnLock(ITEM_ZAIKO_F_SET1)


    ITEM_ZAIKO_F_SET1.MousePointer = vbDefault
