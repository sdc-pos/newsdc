VERSION 5.00
Begin VB.Form F1200901 
   BackColor       =   &H00FFFFFF&
   Caption         =   "出荷実績データ作成 2014.07.10 10:40"
   ClientHeight    =   6960
   ClientLeft      =   2025
   ClientTop       =   2550
   ClientWidth     =   11295
...
'            sts = BTRV(com + BtSNoWait, Y_SYU_POS, Y_SYUREC, Len(Y_SYUREC), K0_Y_SYU, Len(K0_Y_SYU), 0)    2014.07.07
            sts = BTRV(com, Y_SYU_POS, Y_SYUREC, Len(Y_SYUREC), K0_Y_SYU, Len(K0_Y_SYU), 0)                 '2014.07.07
...
'        Loop                                                                                               '2014.07.07
        ...
        If StrConv(Y_SYUREC.JGYOBA, vbUnicode) = "00036003" Then    '2008.01.10
            ...
            If (StrConv(Y_SYUREC.TORI_KBN, vbUnicode) = "25" Or StrConv(Y_SYUREC.TORI_KBN, vbUnicode) = "29") Then
                If StrConv(Y_SYUREC.SYUKA_YMD, vbUnicode) <= Format(Now, "YYYYMMDD") Then    '2006.10.13 日付のﾁｪｯｸ外す   2014.07.10 復活
                                    ...
                                    Do
                                        ...
                                        Select Case sts
                                            ...
                                            Case BtErrRECORD_INUSE, BtErrFILE_INUSE
                                                ...
                                                'レコード/ファイルがロックされている
                                                '画面に表示
                                                lbloUT_Cnt = Format(Out_Cnt, "#0") & ":Update Error=" & sts
                                                '-----------------                                              2014.07.07
...
Dim sts     As Integer
    ...
    LOG_F = Trim(c)
                                '出荷予定ファイル名取り込み
    If GetIni(App.EXEName, "SYUKA_TXT", App.EXEName, c) Then
        Beep
        MsgBox "出荷実績データファイル名の取得に失敗しました。処理を中止して下さい。"
        End
    End If
    SYUKA_DATA = Trim(c)

                                '連番取り込み
    If GetIni(App.EXEName, "SEQ_NO", App.EXEName, c) Then
        Beep
        MsgBox "連番の取得に失敗しました。処理を中止して下さい。"
        End
    End If
    SEQ_NO = Val(Trim(c))
                                '倉庫ｺｰﾄﾞ取り込み
    If GetIni(App.EXEName, "SOKO_CODE", App.EXEName, c) Then
        Beep
        MsgBox "倉庫ｺｰﾄﾞの取得に失敗しました。処理を中止して下さい。"
        End
    End If
    SOKO_CODE = Trim(c)
