VERSION 5.00
Begin VB.Form F1010411 
   BackColor       =   &H00FFFFFF&
   Caption         =   "初期設定在庫登録（滋賀用）"
   ClientHeight    =   6312
   ClientLeft      =   1920
   ClientTop       =   2436
   ClientWidth     =   11220
...
'　　　　単位のＩ－Ｏになる。
'#################################################################################################
Private Type YUKO_SOKO_TBL             '有効ﾎｽﾄ倉庫取り込みテーブル（掃除機事業部）
    HS_SOKO As String * 2
    NAIGAI As String * 1
End Type
Dim SOKO_T7(0 To 9) As YUKO_SOKO_TBL
...
Dim NG_QTY7 As Long                     '無効データ件数（掃除機）
                                    '画面初期表示（処理済「便」表示など）
...
Dim Cp_To As String
    ...
    If Data_Load = False Then       '便別取込みワークにﾃﾞｰﾀﾛｰﾄﾞ
        ...
        End If
                            '印刷フォント設定
        Printer.Font.NAME = F1010411.Font.NAME
        Printer.Font.Size = F1010411.Font.Size
...
'画面項目ロック解除　＆　終了メッセージ表示
    ...
    CntLab(ZERO).Caption = Format(NG_QTY7, "0") & " 件"
...
'掃除機事業部  在庫設定データ取込み
    If HS_ZAI_Open7(ZERO, Work) = False Then   'ﾌｧｲﾙ無しなら処理しない
        Close #HS_ZAI_No

        If HS_ZAI_Open7(1, Work) Then       '掃除機事業部ﾎｽﾄﾃﾞｰﾀ OPEN
            Close #WK_ZAI_No
            Unload Me
        End If

        Call Data_Load_Sub                  '掃除機事業部  在庫設定データ→取込みﾜｰｸ
...
Dim Work As String
    ...
    Do
        If HS_ZAI_Get7() Then          'ﾎｽﾄﾃﾞｰﾀ 読込み
            Exit Do
        End If
...
'ホスト倉庫　範囲外？
        For i = ZERO To UBound(SOKO_T7) - 1
            If SOKO_T7(i).HS_SOKO = "  " Then
                Data_Chk = True
                Exit For
            End If
            If RTrim(StrConv(WK_ZAIREC.HOST_SOKO, vbUnicode)) = RTrim(SOKO_T7(i).HS_SOKO) Then
                Exit For
            End If
        Next i
...
'国内外区分の設定
    For i = ZERO To UBound(SOKO_T7)
        If SOKO_T7(i).HS_SOKO = "  " Then
            Exit For
        End If
        If RTrim(StrConv(WK_ZAIREC.HOST_SOKO, vbUnicode)) = RTrim(SOKO_T7(i).HS_SOKO) Then
            HS_NaiG = SOKO_T7(i).NAIGAI
            Exit For
        End If
    Next i
...
'品目マスタ更新
    ...
    If CLng(StrConv(WK_ZAIREC.ZEN_Z_QTY, vbUnicode)) = ZERO Then   '在庫数＝０→在庫登録無し
        GoTo End_Tran
    End If
...
End Sub
                                            '品目マスタ更新
Private Function Upd_Item() As Integer
Dim sts As Integer
Dim ans As Integer
Dim Command As Integer
Dim Work As String
...
Dim Work As String
    ...
    Select Case Index
        Case ZERO
            Beep
            yn = MsgBox("在庫初期設定　実行しますか？", vbYesNo + vbQuestion, "確認入力")
            If yn = vbYes Then
...
Dim i As Integer
    ...
    F1010411.MousePointer = vbHourglass

    Call Ctrl_Lock(F1010411)
...
Dim i As Integer
    ...
    Call Ctrl_UnLock(F1010411)


    F1010411.MousePointer = vbDefault
