...
Type Y_SYU_TEI_REC_Tag
    ...
    Y_HIN_CHK_CNT(0 To 6)           As Byte         '品番読込み回数                         2012.10.05
    J_HIN_CHK_CNT(0 To 6)           As Byte         '品番読込み済み回数                     2012.10.05
...
Dim c           As String * 128
    ...
    If sts <> False Then
        Call LOG_OUT(LOG_F, "SYS.INI [Y_SYU_TEI]読み込みエラー ")
        Exit Function
    End If

    FullPath = RTrim(c)

    Y_SYU_TEI_Speck.fs.recoleng = Len(Y_SYU_TEI_REC)        ' レコード長
    Y_SYU_TEI_Speck.fs.PageSize = Y_SYU_TEI_PG_SIZ          ' ページサイズ
    Y_SYU_TEI_Speck.fs.idexnumb = 4                         ' インデックス数
    Y_SYU_TEI_Speck.fs.fileflag = 0                         ' ファイルフラグ
    Y_SYU_TEI_Speck.fs.reserve = &H0                        ' 予約済み
'-----------------------------------------------
                                                ' キー０
    Y_SYU_TEI_Speck.ks0.keypos = 1                          ' キーポジション
    Y_SYU_TEI_Speck.ks0.keyleng = 8                         ' キー長
    Y_SYU_TEI_Speck.ks0.keyflag = BtKfExt + BtKfSeg         ' キーフラグ
    Y_SYU_TEI_Speck.ks0.keytype = Chr(BtKtString)           ' キータイプ
    Y_SYU_TEI_Speck.ks0.reserve = &H0                       ' 予約済み

    Y_SYU_TEI_Speck.ks1.keypos = 9                          ' キーポジション
    Y_SYU_TEI_Speck.ks1.keyleng = 6                         ' キー長
                                                            ' キーフラグ
    Y_SYU_TEI_Speck.ks1.keyflag = BtKfExt + BtKfSeg
    Y_SYU_TEI_Speck.ks1.keytype = Chr(BtKtString)           ' キータイプ
    Y_SYU_TEI_Speck.ks1.reserve = &H0                       ' 予約済み

    Y_SYU_TEI_Speck.ks2.keypos = 15                         ' キーポジション
    Y_SYU_TEI_Speck.ks2.keyleng = 5                         ' キー長
                                                            ' キーフラグ
    Y_SYU_TEI_Speck.ks2.keyflag = BtKfExt
    Y_SYU_TEI_Speck.ks2.keytype = Chr(BtKtString)           ' キータイプ
    Y_SYU_TEI_Speck.ks2.reserve = &H0                       ' 予約済み
...
'-----------------------------------------------
                                                ' キー１
    Y_SYU_TEI_Speck.ks3.keypos = 255                        ' キーポジション
    Y_SYU_TEI_Speck.ks3.keyleng = 13                        ' キー長
                                                            ' キーフラグ
    Y_SYU_TEI_Speck.ks3.keyflag = BtKfExt + BtKfChg + BtKfDup
    Y_SYU_TEI_Speck.ks3.keytype = Chr(BtKtString)           ' キータイプ
    Y_SYU_TEI_Speck.ks3.reserve = &H0                       ' 予約済み



'-----------------------------------------------
                                                ' キー２
    Y_SYU_TEI_Speck.ks4.keypos = 182                        ' キーポジション
    Y_SYU_TEI_Speck.ks4.keyleng = 6                         ' キー長
                                                            ' キーフラグ
    Y_SYU_TEI_Speck.ks4.keyflag = BtKfExt + BtKfChg + BtKfDup + BtKfSeg
    Y_SYU_TEI_Speck.ks4.keytype = Chr(BtKtString)           ' キータイプ
    Y_SYU_TEI_Speck.ks4.reserve = &H0                       ' 予約済み

    Y_SYU_TEI_Speck.ks5.keypos = 188                        ' キーポジション
    Y_SYU_TEI_Speck.ks5.keyleng = 6                         ' キー長
                                                            ' キーフラグ
    Y_SYU_TEI_Speck.ks5.keyflag = BtKfExt + BtKfChg + BtKfDup
    Y_SYU_TEI_Speck.ks5.keytype = Chr(BtKtString)           ' キータイプ
    Y_SYU_TEI_Speck.ks5.reserve = &H0                       ' 予約済み


'-----------------------------------------------
                                                ' キー３
    Y_SYU_TEI_Speck.ks6.keypos = 570                        ' キーポジション
    Y_SYU_TEI_Speck.ks6.keyleng = 20                        ' キー長
                                                            ' キーフラグ
    Y_SYU_TEI_Speck.ks6.keyflag = BtKfExt + BtKfChg + BtKfDup
    Y_SYU_TEI_Speck.ks6.keytype = Chr(BtKtString)           ' キータイプ
    Y_SYU_TEI_Speck.ks6.reserve = &H0                       ' 予約済み
    ...
    Do
        ...
        Select Case sts
            ...
            Case BtErrFileNotFound
                sts = Y_SYU_TEI_Create()        '邸別注文データ作成
                If sts <> False Then
                    Exit Function
                End If
                sts = BTRV(BtOpOpen, Y_SYU_TEI_POS, Y_SYU_TEI_REC, Len(Y_SYU_TEI_REC), ByVal FullPath, Len(FullPath), Mode)
